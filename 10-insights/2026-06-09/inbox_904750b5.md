...
id: inbox_904750b5
date: 2026-06-09
source_ref: "[[00-inbox/2026-06-09/2200-codex-releases-0-139-0-4684]]"
title: "0.139.0"
url: https://github.com/openai/codex/releases/tag/rust-v0.139.0
source: codex-releases
published_at: 2026-06-09T20:14:25+00:00
fetched_at: 2026-06-09T22:04:18.466398+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "OpenAI Codex 0.139.0 發布。Code mode 新增 standalone web search 能力（包括嵌套 JavaScript tool calls），返回純文本搜索結果。Tool 和 connector input schemas 現保留 oneOf 和 allOf，大型 schemas 在 compaction 時保留更多淺層結構，顯著改善 MCP 工具相容性。codex doctor 增強本地報告，包含編輯器和 pager 環境詳情。Plugin marketplace 自動化改進，marketplace list 現支持 JSON 輸出，列表可從緩存遠程目錄快速返回。修復 MCP startup warnings 在 subagent 中重複出現的問題、image edits 路徑解析錯誤等。"
key_points:
  - "Code mode 支持 standalone web search，含嵌套 JavaScript tool calls 和純文本結果"
  - "Tool input schemas 保留 oneOf/allOf，large schema compaction 保留更多層級，提升 MCP 工具相容性"
  - "Plugin marketplace automation 支持結構化 JSON 輸出，marketplace list 快速返回緩存目錄"
tags: [codex, web-search, mcp-tools, schema-preservation]
topics: [agents.mcp]
importance: 4
novelty: 4
insight_quality: 3
...
OpenAI Codex 0.139.0 發布。Code mode 新增 standalone web search 能力（包括嵌套 JavaScript tool calls），返回純文本搜索結果。Tool 和 connector input schemas 現保留 oneOf 和 allOf，大型 schemas 在 compaction 時保留更多淺層結構，顯著改善 MCP 工具相容性。codex doctor 增強本地報告，包含編輯器和 pager 環境詳情。Plugin marketplace 自動化改進，marketplace list 現支持 JSON 輸出，列表可從緩存遠程目錄快速返回。修復 MCP startup warnings 在 subagent 中重複出現的問題、image edits 路徑解析錯誤等。
...
**原文：** [codex-releases](https://github.com/openai/codex/releases/tag/rust-v0.139.0)
