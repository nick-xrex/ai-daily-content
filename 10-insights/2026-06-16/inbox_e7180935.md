...
id: inbox_e7180935
date: 2026-06-16
source_ref: "[[00-inbox/2026-06-16/2200-gitnexus-releases-release-candidate-v1-6-8-rc-47-2852]]"
title: "Release Candidate v1.6.8-rc.47"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.8-rc.47
source: gitnexus-releases
published_at: 2026-06-16T10:16:57+00:00
fetched_at: 2026-06-16T22:04:45.502444+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.8-rc.47 發布，包含 49 項改進（相比 rc.48 缺少 CUDA 語法解析與圖 COPY 重疊最佳化）。新增功能涵蓋：Java Spring 路由註解抽取、多分支索引與分支作用域查詢、控制流圖層（TS/JS）、過程內與跨過程污點分析、運達定義數據依賴層、後支配與控制依賴圖、MCP HTTP 伺服器、最短呼叫路徑追蹤工具、私有 GitHub + Azure DevOps 支援。修復項包括：VECTOR 索引、Docker 資產、大倉庫解析、爆炸半徑估計、WebUI 卡頓等。效能改進：PDG/CFG 流式發射、SSA 稀疏運達定義、圖資料庫發射時間縮減。"
key_points:
  - "新增跨過程污點分析與控制流圖層，增強靜態分析能力（支援 TS/JS 等多語言）"
  - "支援多分支索引、分支作用域查詢、私有 GitHub repos + 企業級 DevOps 平臺，提升企業適配性"
  - "49 項改進聚焦效能優化：PDG/CFG 流式發射、SSA 稀疏算法、圖資料庫發射時間大幅改善"
...
GitNexus v1.6.8-rc.47 發布，包含 49 項改進（相比 rc.48 缺少 CUDA 語法解析與圖 COPY 重疊最佳化）。新增功能涵蓋：Java Spring 路由註解抽取、多分支索引與分支作用域查詢、控制流圖層（TS/JS）、過程內與跨過程污點分析、運達定義數據依賴層、後支配與控制依賴圖、MCP HTTP 伺服器、最短呼叫路徑追蹤工具、私有 GitHub + Azure DevOps 支援。修復項包括：VECTOR 索引、Docker 資產、大倉庫解析、爆炸半徑估計、WebUI 卡頓等。效能改進：PDG/CFG 流式發射、SSA 稀疏運達定義、圖資料庫發射時間縮減。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.8-rc.47)
