...
id: inbox_26a1f6c1
date: 2026-06-16
source_ref: "[[00-inbox/2026-06-16/2200-gitnexus-releases-release-candidate-v1-6-8-rc-48-ed65]]"
title: "Release Candidate v1.6.8-rc.48"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.8-rc.48
source: gitnexus-releases
published_at: 2026-06-16T17:48:23+00:00
fetched_at: 2026-06-16T22:04:45.500260+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.8-rc.48 發布，包含 50+ 項新功能、修復與效能改進。新增功能涵蓋：Java Spring 路由註解抽取、多分支索引與分支作用域查詢、控制流圖層（TS/JS）、過程內與跨過程污點分析、運達定義數據依賴層、後支配與控制依賴圖、MCP HTTP 伺服器（流傳輸 HTTP 及 SSE）、最短呼叫路徑追蹤工具、CUDA 語法解析。修復項包括：VECTOR 索引建立、Docker 資產包含、大倉庫解析崩潰、爆炸半徑低估、WebUI 卡頓、私有 GitHub + Azure DevOps 支援。效能改進：PDG/CFG 圖流式發射、SSA 稀疏運達定義算法、圖資料庫發射時間縮減。"
key_points:
  - "新增跨過程污點分析與控制流圖層，顯著增強靜態代碼分析深度（支援 TS/JS/C++ 等多語言）"
  - "支援多分支索引與分支作用域查詢、私有 GitHub repos + PAT、Azure DevOps Server，擴展企業適配性"
  - "50+ 改進包括 PDG/CFG 流式發射與 SSA 稀疏算法，改善超大倉庫分析效能，圖資料庫發射時間大幅縮減"
...
GitNexus v1.6.8-rc.48 發布，包含 50+ 項新功能、修復與效能改進。新增功能涵蓋：Java Spring 路由註解抽取、多分支索引與分支作用域查詢、控制流圖層（TS/JS）、過程內與跨過程污點分析、運達定義數據依賴層、後支配與控制依賴圖、MCP HTTP 伺服器（流傳輸 HTTP 及 SSE）、最短呼叫路徑追蹤工具、CUDA 語法解析。修復項包括：VECTOR 索引建立、Docker 資產包含、大倉庫解析崩潰、爆炸半徑低估、WebUI 卡頓、私有 GitHub + Azure DevOps 支援。效能改進：PDG/CFG 圖流式發射、SSA 稀疏運達定義算法、圖資料庫發射時間縮減。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.8-rc.48)
