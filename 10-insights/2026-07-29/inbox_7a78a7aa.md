...
id: inbox_7a78a7aa
date: 2026-07-29
source_ref: "[[00-inbox/.../inbox_7a78a7aa]]"
title: "v0.37.0"
...
fetched_at: 2026-07-31T01:32:51.966982+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Repowise v0.37.0 發布，號稱「the UI release」。本版本完成了整個 UI 的 section 設計語言重構（Overview、Commits、Contributors、Code Health、Coverage、Dead Code、Chat、Settings、Decisions），將多卡片布局統一為 section 模式以改進導航。新增 --no-editor-setup 標誌跳過全局 MCP/hook 註冊；Stats 頁面重新設計 coding-rhythm 圖表並命名倉庫發布方式；Architecture Graph 修復性能問題（消除 per-node 和 per-frame work，加載時間從 8 秒大幅減少）。新增 Structurizr DSL export 以支持架構決策記錄；MCP get_answer 按供應商預算（而非全局 30s）、修復 get_health 重複問題。支持 16 個 AST 語言，新增 health-coverage、security-scan、OpenCode provider setup、distill/expand/saved 完整演練文檔。"
key_points:
  - "UI 全面 section 化重構，包括 Overview、Code Health、Coverage、Dead Code、Chat、Settings、Decisions 9 個主要頁面"
  - "Architecture Graph 性能優化消除每幀工作，加載時間大幅下降；Knowledge Graph 詳情面板重構、Structurizr DSL 導出支持"
  - "MCP budget-per-provider 而非全局 30s；修復 get_health 重複、health 分類支持 .mts/.cts；新增 health-coverage、security-scan、distill/expand/saved 完整演練"
tags: [ui-redesign, performance-optimization, code-health, architecture-visualization, mcp-tools]
topics: [agents.mcp]
importance: 3
novelty: 2
insight_quality: 2
...
Repowise v0.37.0 發布，號稱「the UI release」。本版本完成了整個 UI 的 section 設計語言重構（Overview、Commits、Contributors、Code Health、Coverage、Dead Code、Chat、Settings、Decisions），將多卡片布局統一為 section 模式以改進導航。新增 --no-editor-setup 標誌跳過全局 MCP/hook 註冊；Stats 頁面重新設計 coding-rhythm 圖表並命名倉庫發布方式；Architecture Graph 修復性能問題（消除 per-node 和 per-frame work，加載時間從 8 秒大幅減少）。新增 Structurizr DSL export 以支持架構決策記錄；MCP get_answer 按供應商預算（而非全局 30s）、修復 get_health 重複問題。支持 16 個 AST 語言，新增 health-coverage、security-scan、OpenCode provider setup、distill/expand/saved 完整演練文檔。
...
**原文：** [repowise-releases](https://github.com/repowise-dev/repowise/releases/tag/v0.37.0)
