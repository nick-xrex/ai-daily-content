...
id: inbox_67caaa3c
date: 2026-05-04
source_ref: "[[00-inbox/2026-05-04/0014-superpowers-releases-v5-1-0-508b]]"
title: "v5.1.0"
...
fetched_at: 2026-05-26T00:24:06.750528+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "superpowers v5.1.0 大幅重构：移除废弃的 slash commands（/brainstorm、/execute-plan、/write-plan），用户改为直接调用对应 skill；将 superpowers:code-reviewer named agent 合并到 skill 中形成\"单一真实来源\"；worktree skills 重写支持环境检测（GIT_DIR != GIT_COMMON）、用户显式同意机制、5 个 harness 跨平台验证；OpenCode bootstrap content 改为 session-level cache 避免每个 agent step 都调用 fs（性能提升）；新增贡献者指南针对 AI agent 生成的不规范 PR（94% 历史拒绝率）。"
key_points:
  - "Worktree 环境检测流程：先检查 GIT_DIR != GIT_COMMON，若已在 linked worktree 则跳过创建，用户确认后再执行（修复了 subagent-driven-development 的隐式创建问题）"
  - "Bootstrap content 缓存优化：从 fs.existsSync + fs.readFileSync + 正则解析移到 session-level cache（15 个回归测试验证），null sentinel 处理文件缺失情况"
  - "Code Review 单一真实来源：persona、checklist、dispatch template 统一到 skills/requesting-code-review/code-reviewer.md，消除了之前在多处 drift 的问题"
tags: [superpowers, skill-consolidation, worktree-management, agent-workflow, bootstrap-optimization]
topics: []
importance: 4
novelty: 3
insight_quality: 4
...
superpowers v5.1.0 大幅重构：移除废弃的 slash commands（/brainstorm、/execute-plan、/write-plan），用户改为直接调用对应 skill；将 superpowers:code-reviewer named agent 合并到 skill 中形成"单一真实来源"；worktree skills 重写支持环境检测（GIT_DIR != GIT_COMMON）、用户显式同意机制、5 个 harness 跨平台验证；OpenCode bootstrap content 改为 session-level cache 避免每个 agent step 都调用 fs（性能提升）；新增贡献者指南针对 AI agent 生成的不规范 PR（94% 历史拒绝率）。
...
**原文：** [superpowers-releases](https://github.com/obra/superpowers/releases/tag/v5.1.0)
