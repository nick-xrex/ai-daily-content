...
id: inbox_70d680f1
date: 2026-06-19
source_ref: "[[00-inbox/2026-06-19/2200-claude-code-releases-v2-1-183-f77b]]"
title: "v2.1.183"
url: https://github.com/anthropics/claude-code/releases/tag/v2.1.183
source: claude-code-releases
published_at: 2026-06-19T01:20:50+00:00
fetched_at: 2026-06-19T22:07:22.034487+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Claude Code v2.1.183 聚焦安全性和穩定性改進。在 auto mode 安全性方面，系統現在自動阻止 git reset --hard、git checkout --、git clean -fd、git stash drop 等破壞性命令，除非使用者明確要求丟棄本地變更。同時，git commit --amend 在非當次 session 產生的 commit 上被禁用，terraform/pulumi/cdk destroy 也需明確授權特定的 stack。新增 deprecated model 警告機制，在 print mode (-p) 和 agent frontmatter 中顯示模型更新狀態。此版本還修復多個穩定性問題，包括 WebSearch 在 subagent 中返回空結果、tmux pane 啟動延遲、後台任務異常終止等。這些改進強化了開發安全性，降低意外的破壞性操作風險。"
key_points:
  - "Destructive git commands (git reset --hard, git checkout --, git clean -fd, git stash drop) 現被自動阻止，除非使用者明確要求丟棄本地變更"
  - "新增 deprecated model 警告機制，在 print mode (-p) 和 agent frontmatter 中觸發"
  - "修復 WebSearch 在 subagent 中返回空結果、tmux pane 啟動延遲、後台任務異常終止等穩定性問題"
tags: [claude-code, safety, git-security, auto-mode, bug-fixes]
...
Claude Code v2.1.183 聚焦安全性和穩定性改進。在 auto mode 安全性方面，系統現在自動阻止 git reset --hard、git checkout --、git clean -fd、git stash drop 等破壞性命令，除非使用者明確要求丟棄本地變更。同時，git commit --amend 在非當次 session 產生的 commit 上被禁用，terraform/pulumi/cdk destroy 也需明確授權特定的 stack。新增 deprecated model 警告機制，在 print mode (-p) 和 agent frontmatter 中顯示模型更新狀態。此版本還修復多個穩定性問題，包括 WebSearch 在 subagent 中返回空結果、tmux pane 啟動延遲、後台任務異常終止等。這些改進強化了開發安全性，降低意外的破壞性操作風險。
...
**原文：** [claude-code-releases](https://github.com/anthropics/claude-code/releases/tag/v2.1.183)
