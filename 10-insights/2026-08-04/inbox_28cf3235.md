...
id: inbox_28cf3235
date: 2026-08-04
source_ref: "[[00-inbox/2026-08-04/0144-gitnexus-releases-release-candidate-v1-6-10-rc-157-dff0]]"
title: "Release Candidate v1.6.10-rc.157"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.157
source: gitnexus-releases
published_at: 2026-08-04T09:20:13+00:00
fetched_at: 2026-08-05T01:52:47.187841+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.10-rc.157（基于主分支 commit f3b4806）涵蓋核心功能与性能优化。功能層面新增 Docker 支援、HTTP embedding backend（自託管/遠程推理端點）、完整 Swift 語言支援（query 修復、export 檢測、隱式導入、建構子解析）、git commit 後自動重索引（保留 embeddings）、`index` 命令便於註冊現有 .gitnexus/ 文件夾。安全性改進 MCP 啟動兼容性、lazy-load CLI 命令、修復 KuzuDB fork 崩潰。性能優化消除 Go 分析中的 O(n²) scope-capture re-walks（#1915），大倉庫 parse cache 採用 sharding 提升穩定性。修復全局升級 ENOTEMPTY、快取目錄用戶可寫位置、tree-sitter-proto devendor 等問題。CI/CD 擴展 macOS 到跨平台測試矩陣。"
key_points:
  - "Docker 支援、HTTP embedding backend（自託管/遠程端點）、完整 Swift 支援（query 修復、export 檢測、隱式導入、建構子解析）"
  - "O(n²) scope-capture re-walks 在 Go 分析中消除；parse cache sharding 優化大倉庫性能穩定性；git commit auto-reindex with embeddings preservation"
  - "MCP 啟動兼容性改進；KuzuDB fork 崩潰與全局升級 ENOTEMPTY 修復；快取目錄設定用戶可寫位置"
tags: [gitnexus, docker-support, embedding-backend, performance-optimization, mcp]
topics: [agents.mcp]
importance: 4
novelty: 4
insight_quality: 4
...
## Release Candidate v1.6.10-rc.157

GitNexus v1.6.10-rc.157（基于主分支 commit f3b4806）涵蓋核心功能与性能优化。功能層面新增 Docker 支援、HTTP embedding backend（自託管/遠程推理端點）、完整 Swift 語言支援（query 修復、export 檢測、隱式導入、建構子解析）、git commit 後自動重索引（保留 embeddings）、`index` 命令便於註冊現有 .gitnexus/ 文件夾。安全性改進 MCP 啟動兼容性、lazy-load CLI 命令、修復 KuzuDB fork 崩潰。性能優化消除 Go 分析中的 O(n²) scope-capture re-walks（#1915），大倉庫 parse cache 採用 sharding 提升穩定性。修復全局升級 ENOTEMPTY、快取目錄用戶可寫位置、tree-sitter-proto devendor 等問題。CI/CD 擴展 macOS 到跨平台測試矩陣。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.157)
...
Automated release candidate build from main .\n\n npm: npm install gitnexus@rc \n Version: 1.6.10-rc.157 \n Target base: 1.6.10 (rc #157 )\n Source commit (main): f3b4806 \n Release commit (versioned tree): 5e8f823 \n\nRelease candidates are pre-stable builds intended for early testing. Stable releases remain on the latest dist-tag. 
