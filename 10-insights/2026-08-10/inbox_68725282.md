...
id: inbox_68725282
date: 2026-08-10
source_ref: "[[00-inbox/2026-08-10/2207-gitnexus-releases-release-candidate-v1-6-10-rc-187-9120]]"
title: "Release Candidate v1.6.10-rc.187"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.187
source: gitnexus-releases
published_at: 2026-08-10T21:43:27+00:00
fetched_at: 2026-08-11T00:10:17.666280+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.10-rc.187 是針對 main 分支最新提交（commit 414c1a5）的自動化 RC 構建。主要安全改善包括 MCP 啟動相容性、CLI 命令延遲加載，並修復 KuzuDB fork crashes。功能新增涵蓋自動重新索引（嵌入保留）、HTTP 嵌入後端（自託管/遠端端點支援）、完整 Swift 語言支援（涵蓋查詢修復、導出偵測、隱式導入、構造函數解析）、Docker 容器化、'index' CLI 指令。性能優化解決 Go 作用域捕獲的 O(n²) 重新遍歷。Bug 修復涵蓋 ENOTEMPTY 全局升級預清理、快取目錄位置修正、大倉庫解析快取分片持久化。CI/CD 新增 macOS 跨平台測試矩陣。"
key_points:
  - "MCP 啟動相容性改善 & CLI 延遲加載；HTTP 嵌入後端支援自託管/遠端端點；KuzuDB fork crash 修復"
  - "完整 Swift 語言支援（export detection、implicit imports、constructor resolution）；Docker 支援；'index' CLI 指令註冊現有索引資料夾"
  - "Go 作用域 O(n²) 複雜度優化（resolves #1848）；ENOTEMPTY 全局升級預清理；大倉庫快取分片持久化"
...
GitNexus v1.6.10-rc.187 是針對 main 分支最新提交（commit 414c1a5）的自動化 RC 構建。主要安全改善包括 MCP 啟動相容性、CLI 命令延遲加載，並修復 KuzuDB fork crashes。功能新增涵蓋自動重新索引（嵌入保留）、HTTP 嵌入後端（自託管/遠端端點支援）、完整 Swift 語言支援（涵蓋查詢修復、導出偵測、隱式導入、構造函數解析）、Docker 容器化、'index' CLI 指令。性能優化解決 Go 作用域捕獲的 O(n²) 重新遍歷。Bug 修復涵蓋 ENOTEMPTY 全局升級預清理、快取目錄位置修正、大倉庫解析快取分片持久化。CI/CD 新增 macOS 跨平台測試矩陣。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.187)
