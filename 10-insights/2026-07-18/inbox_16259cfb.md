...
id: inbox_16259cfb
date: 2026-07-18
source_ref: "[[00-inbox/.../inbox_16259cfb]]"
title: "Release Candidate v1.6.10-rc.49"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.49
source: gitnexus-releases
published_at: 2026-07-18T22:15:33+00:00
fetched_at: 2026-07-20T00:37:28.366912+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.10-rc.49 發佈，新增 CodeBuddy 和 Qoder 編碼代理整合、完整 Codex 支援（hooks、plugin marketplace、setup）。核心改進為「決定性和 MCP 政策系列」（PR #2458–#2482），引入 fail-closed 唯讀模式、標準化 impact/context 參數別名、實施決定性響應預算控制。修復範疇跨越 18+ 項 Windows 穩定性問題：FTS 擴展診斷、HTTP embeddings 自癒、tree-sitter NUL 字節恢復、Napi::Error SIGABRT 工作線程終止管理、LadybugDB 緩衝區上限設置（取代原生 80%-of-RAM 預設）。另包含符號內容 0-based 行儲存與 1-based MCP 顯示轉換、HTTP embedding endpoint 失敗明確報告、Cypher 錯誤診斷。"
key_points:
  - "CodeBuddy、Qoder 代理整合 + 完整 Codex（hooks、plugin marketplace）支援"
  - "MCP 政策系列（PR #2458–#2482）：fail-closed 唯讀模式、參數別名標準化、決定性響應預算控制"
  - "Windows 穩定性修復套件：FTS 診斷、embeddings 自癒、NUL 字節 tree-sitter 恢復、Napi::Error SIGABRT 處理、LadybugDB 緩衝區上限"
tags: [gitnexus-release, mcp-protocol, codex-integration, windows-stability]
topics: [agents.mcp]
importance: 4
novelty: 2
insight_quality: 2
...
## Release Candidate v1.6.10-rc.49

GitNexus v1.6.10-rc.49 發佈，新增 CodeBuddy 和 Qoder 編碼代理整合、完整 Codex 支援（hooks、plugin marketplace、setup）。核心改進為「決定性和 MCP 政策系列」（PR #2458–#2482），引入 fail-closed 唯讀模式、標準化 impact/context 參數別名、實施決定性響應預算控制。修復範疇跨越 18+ 項 Windows 穩定性問題：FTS 擴展診斷、HTTP embeddings 自癒、tree-sitter NUL 字節恢復、Napi::Error SIGABRT 工作線程終止管理、LadybugDB 緩衝區上限設置（取代原生 80%-of-RAM 預設）。另包含符號內容 0-based 行儲存與 1-based MCP 顯示轉換、HTTP embedding endpoint 失敗明確報告、Cypher 錯誤診斷。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.49)
...
Automated release candidate build from main .\n\n npm: npm install gitnexus@rc \n Version: 1.6.10-rc.49 \n Target base: 1.6.10 (rc #49 )\n Source commit (main): 196095b \n Release commit (versioned tree): 9b809f9 \n\nRelease candidates are pre-stable builds intended for early testing. Stable releases remain on the latest dist-tag. 
 ...
 Full Changelog : v1.6.9...v1.6.10-rc.49
