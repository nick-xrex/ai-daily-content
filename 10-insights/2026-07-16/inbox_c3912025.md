...
id: inbox_c3912025
date: 2026-07-16
source_ref: "[[00-inbox/.../inbox_c3912025]]"
...
fetched_at: 2026-07-18T01:40:57.712944+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "月光船 AI 宣佈 Kimi K3，2.8 trillion parameters，定位首個開放 3T 級模型（超越 DeepSeek 1.6T v4 Pro）。自報 benchmark 多數超越 Claude Opus 4.8 / GPT-5.5 high，但輸給 Claude Fable 5 / GPT-5.6 Sol；Artificial Analysis 評估 Elo 1547，+732 vs K2.6，僅次 Fable 5；成本 $0.94/task，同 GPT-5.6 Sol，優於 Opus 4.8 ($1.80)。定價 $3/$15 per million tokens（同 Claude Sonnet 系列），比 K2.6 ($0.95/$4) 高，open weights 版本承諾 7/27 前發佈。K3 特徵：輸出 token 較 K2.6 減 21%；目前唯一 reasoning level 為 max，生成 SVG pelican 時消耗 13,241 reasoning token（成本 $0.25）；tokenizer 計數獨特（\"hi\" = 86 tokens），暗示 ~85 token 隱藏系統提示。作者用 pelican benchmark 測試揭露 K3 的核心限制：無法評估 agentic tool calling 和 long-context 可靠性，這才是 frontier 模型的真正差異。"
key_points:
  - "2.8 trillion parameters（MoE 架構），定價 $3/$15M tokens，同 Claude Sonnet，比 DeepSeek v4 Pro (1.6T) 更大"
  - "Elo 1547 (Artificial Analysis)，+732 vs Kimi K2.6，僅次 Claude Fable 5；成本 $0.94/task；輸出 token 較 K2.6 減 21%"
  - "Max reasoning effort 唯一模式：生成 SVG pelican 需 13,241 reasoning tokens 輸出 3,417 tokens（比例 3.8:1），成本 $0.25；tokenizer 有 ~85 token 隱藏系統提示"
tags: [kimi-k3, moonshot-ai, frontier-models, reasoning-models, llm-pricing]
topics: []
importance: 4
novelty: 4
insight_quality: 4
...
月光船 AI 宣佈 Kimi K3，2.8 trillion parameters，定位首個開放 3T 級模型（超越 DeepSeek 1.6T v4 Pro）。自報 benchmark 多數超越 Claude Opus 4.8 / GPT-5.5 high，但輸給 Claude Fable 5 / GPT-5.6 Sol；Artificial Analysis 評估 Elo 1547，+732 vs K2.6，僅次 Fable 5；成本 $0.94/task，同 GPT-5.6 Sol，優於 Opus 4.8 ($1.80)。定價 $3/$15 per million tokens（同 Claude Sonnet 系列），比 K2.6 ($0.95/$4) 高，open weights 版本承諾 7/27 前發佈。K3 特徵：輸出 token 較 K2.6 減 21%；目前唯一 reasoning level 為 max，生成 SVG pelican 時消耗 13,241 reasoning token（成本 $0.25）；tokenizer 計數獨特（"hi" = 86 tokens），暗示 ~85 token 隱藏系統提示。作者用 pelican benchmark 測試揭露 K3 的核心限制：無法評估 agentic tool calling 和 long-context 可靠性，這才是 frontier 模型的真正差異。
...
**原文：** [simon-willison](https://simonwillison.net/2026/Jul/16/kimi-k3/#atom-everything)
