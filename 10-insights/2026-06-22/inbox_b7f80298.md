...
id: inbox_b7f80298
date: 2026-06-22
source_ref: "[[00-inbox/2026-06-22/2200-simon-willison-porting-the-moebius-0-2b-image-inpaintin-dfd5]]"
...
published_at: 2026-06-22T23:43:51+00:00
fetched_at: 2026-06-23T22:08:01.624024+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Simon Willison 使用 Claude Code 將 Moebius 0.2B 圖像修復模型（原本需 PyTorch + NVIDIA CUDA）轉換至 ONNX 格式並透過 WebGPU 在瀏覽器執行。完整工作流包括：(1) 使用 Claude 進行初步可行性研究，(2) Claude Code 自動完成模型轉換，(3) 發布 1.24GB 轉換後的 ONNX 權重至 Hugging Face，(4) 構建並部署前端界面至 GitHub Pages（simonw.github.io/moebius-web/），(5) 解決瀏覽器快取問題（採用 CacheStorage API）。此項目展示 Claude Opus 4.8 作為編碼代理的能力：自動處理模型轉換、第三方平台發布、跨瀏覽器相容性優化。整個過程耗時數小時，主要時間用於代理的並行執行與反覆除錯。"
key_points:
  - "模型自動轉換與發布：Claude Code 無需人工干預，完成 PyTorch → ONNX 轉換，部署 1.24GB 權重至 Hugging Face"
  - "瀏覽器推理可行性：Chrome、Firefox、Safari 均可運行 0.2B 圖像修復模型，WebGPU 性能可達實用水準"
  - "快取優化決策：識別並解決瀏覽器每次重載均下載 1.3GB 的問題，使用 CacheStorage API 實現持久快取"
tags: [moebius, onnx, webgpu, model-conversion, browser-inference]
topics: []
importance: 4
novelty: 4
insight_quality: 4
...
Simon Willison 使用 Claude Code 將 Moebius 0.2B 圖像修復模型（原本需 PyTorch + NVIDIA CUDA）轉換至 ONNX 格式並透過 WebGPU 在瀏覽器執行。完整工作流包括：(1) 使用 Claude 進行初步可行性研究，(2) Claude Code 自動完成模型轉換，(3) 發布 1.24GB 轉換後的 ONNX 權重至 Hugging Face，(4) 構建並部署前端界面至 GitHub Pages（simonw.github.io/moebius-web/），(5) 解決瀏覽器快取問題（採用 CacheStorage API）。此項目展示 Claude Opus 4.8 作為編碼代理的能力：自動處理模型轉換、第三方平台發布、跨瀏覽器相容性優化。整個過程耗時數小時，主要時間用於代理的並行執行與反覆除錯。
...
**原文：** [simon-willison](https://simonwillison.net/2026/Jun/22/porting-moebius/#atom-everything)
