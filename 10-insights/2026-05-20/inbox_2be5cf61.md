...
id: inbox_2be5cf61
date: 2026-05-20
source_ref: "[[00-inbox/2026-05-20/0917-codex-releases-0-132-0-2944]]"
title: "0.132.0"
url: https://github.com/openai/codex/releases/tag/rust-v0.132.0
source: codex-releases
published_at: 2026-05-20T02:36:51+00:00
fetched_at: 2026-05-21T09:21:44.240530+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "OpenAI Codex 發布 v0.132.0，Python SDK 新增第一級認證支持（API key、ChatGPT 瀏覽器及設備代碼流程）、turn API 簡化可直接傳遞純字符串並回傳更豐富的 TurnResult（含 timing、usage 數據）；codex exec resume 指令支持 --output-schema 參數以保留會話上下文同時強制結構化 JSON 輸出；TUI 啟動效能提升（終端能力檢測批次化）、應用伺服器保留圖片保真度；修復多個 TUI 流程、多會話、Windows 安裝及記憶體摘要版本控制問題。"
key_points:
  - "Python SDK 新增第一級認證（API key、device-code、ChatGPT 瀏覽器登入）"
  - "turn API 簡化：支持字符串輸入、回傳更豐富的 TurnResult 含 timing 和 usage"
  - "codex exec resume 支持 --output-schema 參數保留會話上下文與結構化輸出"
tags: [openai, codex, python-sdk, release, authentication]
...
OpenAI Codex 發布 v0.132.0，Python SDK 新增第一級認證支持（API key、ChatGPT 瀏覽器及設備代碼流程）、turn API 簡化可直接傳遞純字符串並回傳更豐富的 TurnResult（含 timing、usage 數據）；codex exec resume 指令支持 --output-schema 參數以保留會話上下文同時強制結構化 JSON 輸出；TUI 啟動效能提升（終端能力檢測批次化）、應用伺服器保留圖片保真度；修復多個 TUI 流程、多會話、Windows 安裝及記憶體摘要版本控制問題。
...
**原文：** [codex-releases](https://github.com/openai/codex/releases/tag/rust-v0.132.0)
