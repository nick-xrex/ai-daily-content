...
id: inbox_1d810ec8
date: 2026-06-15
source_ref: "[[00-inbox/.../inbox_1d810ec8]]"
title: "0.140.0"
url: https://github.com/openai/codex/releases/tag/rust-v0.140.0
source: codex-releases
published_at: 2026-06-15T21:08:11+00:00
fetched_at: 2026-06-16T00:43:14.374815+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "OpenAI Codex 發布 v0.140.0，新增 /usage 視圖用於追蹤日度/週度/累積 token 使用量、/import 指令從 Claude Code 匯入設定、@ mention 統一菜單選擇檔案/外掛/技能。重點改進包括 SQLite 資料庫自動備份與復原機制、MCP 瞬態失敗自動重試、OAuth 憑證失敗狀態通報、禁用伺服器設定保留。針對大型儲存庫最佳化 Git fsmonitor 保留與檔案歷史快取。並修復 remote plugin 卸載、CJK 輸入法、vim 模式、background session 等多項問題。"
key_points:
  - "/usage 指令提供日度/週度/累積 token 使用統計，支援遠端 app-server 會話；SQLite 損毀自動備份與重建機制實現無需手動介入的可靠性恢復"
  - "Amazon Bedrock API 密鑰管理認證搭配加密本地儲存，降低憑證管理開銷與安全風險"
  - "MCP 連接瞬態失敗自動重試、不可用 OAuth 通報為已登出、保留明確禁用伺服器設定，整體提升連線穩定性"
tags: [codex, openai-code, token-tracking, reliability, mcp]
topics: []
importance: 3
novelty: 2
insight_quality: 3
...
OpenAI Codex 發布 v0.140.0，新增 /usage 視圖用於追蹤日度/週度/累積 token 使用量、/import 指令從 Claude Code 匯入設定、@ mention 統一菜單選擇檔案/外掛/技能。重點改進包括 SQLite 資料庫自動備份與復原機制、MCP 瞬態失敗自動重試、OAuth 憑證失敗狀態通報、禁用伺服器設定保留。針對大型儲存庫最佳化 Git fsmonitor 保留與檔案歷史快取。並修復 remote plugin 卸載、CJK 輸入法、vim 模式、background session 等多項問題。
...
**原文：** [codex-releases](https://github.com/openai/codex/releases/tag/rust-v0.140.0)
...
New Features 
 ...
 #27670 Make MCP server contributions t
