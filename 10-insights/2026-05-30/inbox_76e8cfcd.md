...
id: inbox_76e8cfcd
date: 2026-05-30
source_ref: "[[00-inbox/2026-05-30/0039-ruflo-releases-v3-10-28-lucene-bm25-rrf-ce-rerank-passe-4847]]"
title: "v3.10.28 — Lucene BM25 + RRF + CE rerank — passes acceptance test on BOTH datasets (rank 3/13 on 2-dataset mean)"
url: https://github.com/ruvnet/ruflo/releases/tag/v3.10.28
source: ruflo-releases
published_at: 2026-05-30T23:24:25+00:00
fetched_at: 2026-05-31T00:45:17.089143+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Ruflo v3.10.28 通過實現 Lucene 質量的 BM25（Porter 1980 stemmer + Lucene stopwords + length norm）修復 v3.10.27 RRF 失敗問題，達成 BEIR 2-dataset 排名 3/13（nDCG@10 平均 0.521）。核心診斷驗證：v3.10.27 RRF 惡化源於非對稱輸入強度（自製 BM25 0.279 vs Lucene 基準 0.325），新版 BM25 與基準 ±0.003 內匹配後，RRF+cross-encoder rerank 恢復真實收益。Per-dataset 結果：NFCorpus 0.358（排名 2/11）、SciFact 0.683（排名 3/11），僅用 110M 模型體量對比 BGE-large 的 335M。細微發現：NFCorpus 上 CE rerank 無加值（RRF 已強），但 SciFact 實現 +0.044 lift（0.639→0.683），Pipeline 動態適配。"
key_points:
  - "Lucene BM25 實現（Porter stemmer+Lucene stopwords，全量測試通過）修復非對稱輸入問題，RRF+CE rerank 達 BEIR 排名 3/13（nDCG@10 0.521），110M 模型體量優於 BGE-large 335M 與 GTR-XL 1.2B"
  - "診斷驗證：BM25 0.279→0.325 對齊後，RRF 才能發揮設計效能，證實『輸入系統質量相當』是 RRF 必要條件"
  - "Cross-encoder rerank 價值因任務異質：NFCorpus 無加值，SciFact +0.044，Pipeline 自適應調整符合已發表文獻"
tags: [search-ranking, bm25, rrf, cross-encoder, beir]
topics: []
importance: 4
novelty: 4
insight_quality: 5
...
Ruflo v3.10.28 通過實現 Lucene 質量的 BM25（Porter 1980 stemmer + Lucene stopwords + length norm）修復 v3.10.27 RRF 失敗問題，達成 BEIR 2-dataset 排名 3/13（nDCG@10 平均 0.521）。核心診斷驗證：v3.10.27 RRF 惡化源於非對稱輸入強度（自製 BM25 0.279 vs Lucene 基準 0.325），新版 BM25 與基準 ±0.003 內匹配後，RRF+cross-encoder rerank 恢復真實收益。Per-dataset 結果：NFCorpus 0.358（排名 2/11）、SciFact 0.683（排名 3/11），僅用 110M 模型體量對比 BGE-large 的 335M。細微發現：NFCorpus 上 CE rerank 無加值（RRF 已強），但 SciFact 實現 +0.044 lift（0.639→0.683），Pipeline 動態適配。
...
**原文：** [ruflo-releases](https://github.com/ruvnet/ruflo/releases/tag/v3.10.28)
