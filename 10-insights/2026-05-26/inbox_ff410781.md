...
id: inbox_ff410781
date: 2026-05-26
source_ref: "[[00-inbox/2026-05-26/0022-codex-releases-0-134-0-3929]]"
title: "0.134.0"
url: https://github.com/openai/codex/releases/tag/rust-v0.134.0
source: codex-releases
published_at: 2026-05-26T19:31:37+00:00
fetched_at: 2026-05-27T00:27:37.174713+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "OpenAI Codex 發布 v0.134.0 版本。主要新功能包括本地對話歷史搜索（支持大小寫不敏感匹配與結果預覽）、--profile 統一作為主要配置選擇器（淘汰舊版本設定）。MCP 設置改進包括顯式環境路由與 OAuth 選項支持，connector 工具 schemas 保留本地 $ref/$defs 結構並支持大型 schemas 壓縮。讀取類 MCP 工具可在聲明 readOnlyHint 時並發執行。Bug 修復涵蓋遠程 websocket 斷線重連、Windows TUI 虛擬終端模式修復、workspace 特定使用限制訊息。開發者文檔新增 curl 和 PowerShell 安裝路徑、profile 遷移指引。"
key_points:
  - "本地對話歷史搜索新增，支持大小寫不敏感匹配和結果預覽（#23519, #23921）"
  - "--profile 統一為主配置選擇器，淘汰舊版本配置方式（#23708, #23883, #23890 等）"
  - "MCP 工具讀取並發執行支持與 connector schemas 壓縮優化（#23750, #23904）"
tags: [openai-codex, mcp-setup, release, development-tools, bug-fixes]
topics: []
importance: 3
novelty: 2
insight_quality: 2
...
OpenAI Codex 發布 v0.134.0 版本。主要新功能包括本地對話歷史搜索（支持大小寫不敏感匹配與結果預覽）、--profile 統一作為主要配置選擇器（淘汰舊版本設定）。MCP 設置改進包括顯式環境路由與 OAuth 選項支持，connector 工具 schemas 保留本地 $ref/$defs 結構並支持大型 schemas 壓縮。讀取類 MCP 工具可在聲明 readOnlyHint 時並發執行。Bug 修復涵蓋遠程 websocket 斷線重連、Windows TUI 虛擬終端模式修復、workspace 特定使用限制訊息。開發者文檔新增 curl 和 PowerShell 安裝路徑、profile 遷移指引。
...
**原文：** [codex-releases](https://github.com/openai/codex/releases/tag/rust-v0.134.0)
