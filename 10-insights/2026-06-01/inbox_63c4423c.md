...
id: inbox_63c4423c
date: 2026-06-01
source_ref: "[[00-inbox/2026-06-01/2245-gitnexus-releases-release-candidate-v1-6-6-rc-110-b76a]]"
title: "Release Candidate v1.6.6-rc.110"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.6-rc.110
source: gitnexus-releases
published_at: 2026-06-01T20:49:38+00:00
fetched_at: 2026-06-01T22:49:58.994161+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.6-rc.110 發布候選版本包含安全與功能增強。安全面，MCP startup 兼容性改進、CLI 指令 lazy-load 減少初始負荷。功能上，git commit 後自動 reindex 並保留 embedding、支援 HTTP embedding backend 供遠端或自架端點、Swift 語言支援補全（含 query fix、export 偵測、implicit import、constructor 解析）、Docker 容器支援。效能上消除 Go scope-capture 的 O(n²) 重走過程。Bug fix 包括 preinstall cleanup 防止 ENOTEMPTY、cache dir 改為使用者可寫位置、tree-sitter-proto devendor 避免升級衝突、大型 repo parse cache 分片儲存。"
key_points:
  - "Git commit hook 整合：自動觸發 reindex 同時保留 embedding，無須手動更新索引"
  - "多語言擴展：完整 Swift support（含 iOS 整合）、Docker 容器化、HTTP 遠端 embedding backend"
  - "效能與穩定性：O(n²) scope-capture 消除、preinstall 清理防止 ENOTEMPTY、parse cache 分片應對大型 repo"
...
## Release Candidate v1.6.6-rc.110

GitNexus v1.6.6-rc.110 發布候選版本包含安全與功能增強。安全面，MCP startup 兼容性改進、CLI 指令 lazy-load 減少初始負荷。功能上，git commit 後自動 reindex 並保留 embedding、支援 HTTP embedding backend 供遠端或自架端點、Swift 語言支援補全（含 query fix、export 偵測、implicit import、constructor 解析）、Docker 容器支援。效能上消除 Go scope-capture 的 O(n²) 重走過程。Bug fix 包括 preinstall cleanup 防止 ENOTEMPTY、cache dir 改為使用者可寫位置、tree-sitter-proto devendor 避免升級衝突、大型 repo parse cache 分片儲存。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.6-rc.110)
...
Automated release candidate build from main .\n\n npm: npm install gitnexus@rc \n Version: 1.6.6-rc.110 \n Target base: 1.6.6 (rc #110 )\n Source commit (main): 4f7697c \n Release commit (versioned tree): 639b527 \n\nRelease candidates are pre-stable builds intended for early testing. Stable releases remain on the latest dist-tag. 
