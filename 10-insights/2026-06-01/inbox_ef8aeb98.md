...
id: inbox_ef8aeb98
date: 2026-06-01
source_ref: "[[00-inbox/2026-06-01/2245-codex-releases-0-136-0-8bc8]]"
title: "0.136.0"
url: https://github.com/openai/codex/releases/tag/rust-v0.136.0
source: codex-releases
published_at: 2026-06-01T18:51:30+00:00
fetched_at: 2026-06-01T22:49:58.993140+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "OpenAI Codex v0.136.0 發布含多項功能與安全加強。TUI markdown 保持 OSC 8 web link 可點擊、大表格自動轉為 key/value 易讀格式；sessions 支援 archive/unarchive 防止意外修改。App-server 可在 resume thread 時包含初始 turns page、展示豐富的 MCP server 狀態、支援 stdio 模式。遠端執行現支援 CODEX_API_KEY 註冊，remote-control 改用短生命周期 server token 替代 ChatGPT 存取 token。Windows 管理員可用 alpha codex sandbox setup --elevated。安全性上，command-safety 防止 /diff 執行 repo Git hook、避免非 Windows 主機 PowerShell 執行、拒絕瀏覽器來源 websocket 握手。TUI session resume、Vim 編輯、Bedrock auth fallback、Python SDK 文檔等亦有改進。"
key_points:
  - "TUI 支援 OSC 8 超連結保留、markdown 表格自動降級為 key/value 格式，改善互動性與可讀性"
  - "App-server session resume 包含初始 turns page、MCP server 狀態更豐富、支援 stdio 模式，強化遠端執行能力"
  - "安全加強：command-safety 防止 repo Git hook 執行、remote-control 改用 short-lived server token、Windows sandbox 網路拒絕能可靠清理資源"
tags: [codex, release, tui, app-server, mcp, security, windows]
topics: [agents.mcp]
importance: 4
novelty: 4
insight_quality: 2
...
OpenAI Codex v0.136.0 發布含多項功能與安全加強。TUI markdown 保持 OSC 8 web link 可點擊、大表格自動轉為 key/value 易讀格式；sessions 支援 archive/unarchive 防止意外修改。App-server 可在 resume thread 時包含初始 turns page、展示豐富的 MCP server 狀態、支援 stdio 模式。遠端執行現支援 CODEX_API_KEY 註冊，remote-control 改用短生命周期 server token 替代 ChatGPT 存取 token。Windows 管理員可用 alpha codex sandbox setup --elevated。安全性上，command-safety 防止 /diff 執行 repo Git hook、避免非 Windows 主機 PowerShell 執行、拒絕瀏覽器來源 websocket 握手。TUI session resume、Vim 編輯、Bedrock auth fallback、Python SDK 文檔等亦有改進。
...
**原文：** [codex-releases](https://github.com/openai/codex/releases/tag/rust-v0.136.0)
