...
id: inbox_e0c49834
date: 2026-05-08
source_ref: "[[00-inbox/2026-05-08/0151-reddit-localllama-a-new-generation-of-ai-models-and-one-of-f460]]"
title: "A new generation of AI models and one of the most powerful research papers out there."
...
fetched_at: 2026-05-09T02:10:25.372958+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Token AI 發表 STAM（穩定自適應動量訓練）優化器，改進 Adam/AdamW 的固定動量機制。核心創新：beta1（動量係數）動態調整——梯度嘈雜時降低 beta1 以消除過期動量影響，梯度穩定時保持高動量以加速收斂。STAMLite 輕量版記憶體需求為 1× 參數量（AdamW 為 2×），GPU 訓練耗能降低約 50%。基準測試中長視界非平穩 MLP 任務達 0.97 準度（與 NAdam 並列）。發表者已用 STAM 從零訓練完整 LLM。"
key_points:
  - "STAM 動態調整 beta1：基於梯度方差自動降低或提升動量，避免舊動量在嘈雜訓練期損害收斂"
  - "STAMLite 記憶體開銷僅 1× 參數量，相比 AdamW/STAM-Full 減少 ~50% GPU 耗能"
  - "長視界非平穩 MLP 基準：0.97 準度；HyperParameter Sweep：0.61 準度、0.91 損失"
tags: [stam-optimizer, adaptive-momentum, training-optimization, llm-training]
topics: []
importance: 4
novelty: 4
insight_quality: 5
...
Token AI 發表 STAM（穩定自適應動量訓練）優化器，改進 Adam/AdamW 的固定動量機制。核心創新：beta1（動量係數）動態調整——梯度嘈雜時降低 beta1 以消除過期動量影響，梯度穩定時保持高動量以加速收斂。STAMLite 輕量版記憶體需求為 1× 參數量（AdamW 為 2×），GPU 訓練耗能降低約 50%。基準測試中長視界非平穩 MLP 任務達 0.97 準度（與 NAdam 並列）。發表者已用 STAM 從零訓練完整 LLM。
...
**原文：** [reddit-localllama](https://www.reddit.com/r/LocalLLaMA/comments/1t6yra2/a_new_generation_of_ai_models_and_one_of_the_most/)
