...
id: inbox_6af3866e
date: 2026-06-04
source_ref: "[[00-inbox/.../inbox_6af3866e]]"
title: "Release Candidate v1.6.6-rc.143"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.6-rc.143
source: gitnexus-releases
published_at: 2026-06-04T22:27:05+00:00
fetched_at: 2026-06-05T01:15:46.254410+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.6-rc.143 RC 發佈聚焦多語言 AST 智能和嵌入式管道。新增 git commit 自動重新索引（保留嵌入）、HTTP 遠端嵌入端點（自建/雲端）、Swift/Kotlin/Ruby/PHP 完整語言支援、Docker 部署、CLI index 命令；修復 Go 分析 O(n²) scope-capture 迴歸、快取分片持久化、全局升級 ENOTEMPTY 錯誤；從 KuzuDB 遷移至 LadybugDB v0.15；八階段 type resolution 框架涵蓋 10+ 語言（型別推論、模式匹配、容器描述符、欄位推論、for-loop 型別化、鏈式呼叫、隱式導入）；MCP 啟動相容性和多倉庫協調改進。"
key_points:
  - "多階段 type resolution 架構（Phase 1–8）：迴圈型別推論、模式匹配、容器描述符、欄位/屬性型別推論，支援 Python/JavaScript/Java/Go/C++/Rust/Swift/Kotlin/Ruby/PHP"
  - "自動 git-commit 重新索引保留嵌入；HTTP 遠端端點（自建或雲端提供者）"
  - "效能優化：O(n²) scope-capture 消除；快取分片持久化；Docker 支援；macOS 跨平台 CI 擴展"
tags: [gitnexus, type-resolution, multi-language, embeddings, docker]
topics: [agents.mcp]
importance: 4
novelty: 4
insight_quality: 4
...
## Release Candidate v1.6.6-rc.143

GitNexus v1.6.6-rc.143 RC 發佈聚焦多語言 AST 智能和嵌入式管道。新增 git commit 自動重新索引（保留嵌入）、HTTP 遠端嵌入端點（自建/雲端）、Swift/Kotlin/Ruby/PHP 完整語言支援、Docker 部署、CLI index 命令；修復 Go 分析 O(n²) scope-capture 迴歸、快取分片持久化、全局升級 ENOTEMPTY 錯誤；從 KuzuDB 遷移至 LadybugDB v0.15；八階段 type resolution 框架涵蓋 10+ 語言（型別推論、模式匹配、容器描述符、欄位推論、for-loop 型別化、鏈式呼叫、隱式導入）；MCP 啟動相容性和多倉庫協調改進。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.6-rc.143)
...
Automated release candidate build from main .\n\n npm: npm install gitnexus@rc \n Version: 1.6.6-rc.143 \n Target base: 1.6.6 (rc #143 )\n Source commit (main): 54ea21a \n Release commit (versioned tree): e258905 \n\nRelease candidates are pre-stable builds intended for early testing. Stable releases remain on the latest dist-tag. 
