...
id: inbox_9f511555
date: 2026-06-04
source_ref: "[[00-inbox/.../inbox_9f511555]]"
title: "0.137.0"
url: https://github.com/openai/codex/releases/tag/rust-v0.137.0
source: codex-releases
published_at: 2026-06-04T01:38:37+00:00
fetched_at: 2026-06-05T01:15:46.252263+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Codex v0.137.0 聚焦企業功能、多代理系統和基礎設施可靠性。新增 F13–F24 按鍵綁定、可搜尋菜單中的粘貼、推理專用狀態顯示；企業流程支援月度額度限制和雲端托管配置套件（含 EDU workspace）；多代理 v2 維持執行時選擇、改進後續任務和元數據預設；遠端控制客戶端支援配對啟動和授權管理；大量穩定性修復（macOS 深度連結、Windows SQLite、並行工具呼叫失敗隔離、環境身份追蹤）。"
key_points:
  - "企業級：月度額度限制、雲端配置套件（EDU workspace 支援）、環境身份權限追蹤"
  - "多代理 v2：執行時持久化、清晰後續任務 API、元數據預設值、每執行緒執行時選擇"
  - "基礎設施：macOS 深度連結修復、Windows SQLite 內在函數禁用、並行工具呼叫失敗隔離、會話履歷壓縮"
tags: [codex, enterprise, multi-agent, remote-control]
topics: []
importance: 4
novelty: 3
insight_quality: 3
...
Codex v0.137.0 聚焦企業功能、多代理系統和基礎設施可靠性。新增 F13–F24 按鍵綁定、可搜尋菜單中的粘貼、推理專用狀態顯示；企業流程支援月度額度限制和雲端托管配置套件（含 EDU workspace）；多代理 v2 維持執行時選擇、改進後續任務和元數據預設；遠端控制客戶端支援配對啟動和授權管理；大量穩定性修復（macOS 深度連結、Windows SQLite、並行工具呼叫失敗隔離、環境身份追蹤）。
...
**原文：** [codex-releases](https://github.com/openai/codex/releases/tag/rust-v0.137.0)
