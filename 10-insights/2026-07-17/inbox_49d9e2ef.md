...
id: inbox_49d9e2ef
date: 2026-07-17
source_ref: "[[00-inbox/.../inbox_49d9e2ef]]"
title: "v2.1.212"
url: https://github.com/anthropics/claude-code/releases/tag/v2.1.212
source: claude-code-releases
published_at: 2026-07-17T00:26:27+00:00
fetched_at: 2026-07-18T01:40:57.737773+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Claude Code v2.1.212 是大規模 maintenance release，涵蓋 40+ 項修復、安全加強、功能改善，跨越安全、穩定性、性能、可觀測性四大維度。核心主題：(1) 背景 session 管理升級（/fork 複製對話到獨立 session、/resume 選擇器含已刪除 session、MCP tool > 2 分鐘自動背景化），(2) runaway prevention 限流保護（WebSearch 200/session、Subagent 200/session、/clear 重置預算），(3) 安全修復 6 項（plan mode Bash 無提示執行、.claude/worktrees symlink 驗證、mTLS transport 忽略、continue:false hook halt、SIGTERM 進程清理、permission bypass 多項），(4) 跨平台穩定性（Windows PowerShell Group Policy、shell 自動完成衝突、unicode 編碼），(5) UX 改善（/ultrareview 遠端分支、emoji 截斷、Ctrl+J newline），(6) 可觀測性增強（OpenTelemetry HTTP 411/400、OTLP trace_id/span_id、message-level correlation）。架構變更：Task tool mode 參數棄用、Headless/SDK mid-turn set_model control request、session transcript 記錄 reasoning effort。"
key_points:
  - "背景 session 管理：/fork 複製對話、/resume session 選擇器、MCP tool > 2min 自動背景化，提升 session 可用性"
  - "Runaway prevention 三層防禦：WebSearch/Subagent 各 200/session 上限、/clear 預算重置、auto-background for long-running，防止代理失控"
  - "安全修復 6 項：plan mode Bash 權限提示、symlink 驗證、mTLS transport、continue:false hook、SIGTERM cleanup、permission bypass 堵漏"
tags: [claude-code, maintenance-release, background-sessions, rate-limiting, security-fixes]
...
Claude Code v2.1.212 是大規模 maintenance release，涵蓋 40+ 項修復、安全加強、功能改善，跨越安全、穩定性、性能、可觀測性四大維度。核心主題：(1) 背景 session 管理升級（/fork 複製對話到獨立 session、/resume 選擇器含已刪除 session、MCP tool > 2 分鐘自動背景化），(2) runaway prevention 限流保護（WebSearch 200/session、Subagent 200/session、/clear 重置預算），(3) 安全修復 6 項（plan mode Bash 無提示執行、.claude/worktrees symlink 驗證、mTLS transport 忽略、continue:false hook halt、SIGTERM 進程清理、permission bypass 多項），(4) 跨平台穩定性（Windows PowerShell Group Policy、shell 自動完成衝突、unicode 編碼），(5) UX 改善（/ultrareview 遠端分支、emoji 截斷、Ctrl+J newline），(6) 可觀測性增強（OpenTelemetry HTTP 411/400、OTLP trace_id/span_id、message-level correlation）。架構變更：Task tool mode 參數棄用、Headless/SDK mid-turn set_model control request、session transcript 記錄 reasoning effort。
...
**原文：** [claude-code-releases](https://github.com/anthropics/claude-code/releases/tag/v2.1.212)
