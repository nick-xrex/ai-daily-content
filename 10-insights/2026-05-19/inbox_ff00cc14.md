...
id: inbox_ff00cc14
date: 2026-05-19
source_ref: "[[00-inbox/2026-05-19/1800-ruflo-releases-v3-7-0-alpha-71-adr-125-memory-consolida-4e73]]"
title: "v3.7.0-alpha.71 — ADR-125 Memory Consolidation (full delivery)"
url: https://github.com/ruvnet/ruflo/releases/tag/v3.7.0-alpha.71
source: ruflo-releases
published_at: 2026-05-19T22:48:00+00:00
fetched_at: 2026-05-22T18:08:46.132131+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Ruflo v3.7.0-alpha.71（ADR-125 完整交付，May 19）@claude-flow/memory 3.0.0-alpha.18 重寫 memory subsystem。核心成就：①單一入點 MemoryService（HnswLite/RvfBackend 移出公開 surface）②真實 hybrid 預設（sql.js + AgentDB，ADR-009 終於交付）③Persistent HNSW 毫秒級恢復（restart 無需重建，自動快照）④MemoryConsolidator 服務（sweepExpired/dedup/compactHnsw，6h timer）。效能優化：graceful degradation（無 embeddings 時 FTS5 fallback，health.embedder='degraded'），RRF + MMR diversity rerank。基準測試可執行（npm run bench 產生 baseline），8/8 acceptance criteria 全數達成，+36 tests、+5 test files，向後相容（UnifiedMemoryService deprecated alias）。"
key_points:
  - "Persistent HNSW 快照恢復：毫秒級冷啟動無需 O(n log n) rebuild，自動快照每 Nth write"
  - "Graceful degradation pattern：無 embeddings 時安全降級到 FTS5（不 throw），health signal 透明"
  - "Hybrid memory architecture（sql.js + AgentDB + HNSW + FTS5）：多層後端組合達成 ADR-009 hybrid 承諾"
tags: [persistent-hnsw, hybrid-memory-backend, graceful-degradation, memory-consolidation, adr-driven]
topics: []
importance: 3
novelty: 3
insight_quality: 4
...
Ruflo v3.7.0-alpha.71（ADR-125 完整交付，May 19）@claude-flow/memory 3.0.0-alpha.18 重寫 memory subsystem。核心成就：①單一入點 MemoryService（HnswLite/RvfBackend 移出公開 surface）②真實 hybrid 預設（sql.js + AgentDB，ADR-009 終於交付）③Persistent HNSW 毫秒級恢復（restart 無需重建，自動快照）④MemoryConsolidator 服務（sweepExpired/dedup/compactHnsw，6h timer）。效能優化：graceful degradation（無 embeddings 時 FTS5 fallback，health.embedder='degraded'），RRF + MMR diversity rerank。基準測試可執行（npm run bench 產生 baseline），8/8 acceptance criteria 全數達成，+36 tests、+5 test files，向後相容（UnifiedMemoryService deprecated alias）。
...
**原文：** [ruflo-releases](https://github.com/ruvnet/ruflo/releases/tag/v3.7.0-alpha.71)
