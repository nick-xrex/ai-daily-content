...
id: inbox_00aef576
date: 2026-05-19
source_ref: "[[00-inbox/2026-05-19/0201-claude-code-releases-v2-1-144-c41a]]"
title: "v2.1.144"
url: https://github.com/anthropics/claude-code/releases/tag/v2.1.144
source: claude-code-releases
published_at: 2026-05-19T00:48:51+00:00
fetched_at: 2026-05-19T02:05:08.862060+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Claude Code v2.1.144 發布，針對後台會話、終端渲染、MCP 伺服器、模型選擇等發布 30+ 項修復。主要改進包括：後台會話支持 /resume 命令，完成通知加入經過時間；修復啟動卡 75 秒的無網路問題（API timeout 改為 15s）；修復 macOS Full Disk Access 保護資料夾下後台會話 exit 1 崩潰；修復 MCP 伺服器分頁工具只返回首頁、圖像格式相容問題；減少虛假工具錯誤並改進終端渲染穩定性；提升 SDK 和無頭環境啟動速度（最快 2s）。該版本為維護型更新，重點聚焦穩定性提升。"
key_points:
  - "修復啟動卡 75s（無網路時 API 側通道 timeout 改為 15s，防止 VPN/防火牆問題）"
  - "支持 /resume 後台會話，完成通知加入經過時間（如 3h 2m 5s）"
  - "修復 macOS Full Disk Access 保護資料夾下後台會話「exit 1 before init」崩潰（v2.1.143 迴歸）"
tags: [claude-code, bug-fixes, terminal-rendering, mcp]
topics: []
importance: 3
novelty: 2
insight_quality: 2
...
Claude Code v2.1.144 發布，針對後台會話、終端渲染、MCP 伺服器、模型選擇等發布 30+ 項修復。主要改進包括：後台會話支持 /resume 命令，完成通知加入經過時間；修復啟動卡 75 秒的無網路問題（API timeout 改為 15s）；修復 macOS Full Disk Access 保護資料夾下後台會話 exit 1 崩潰；修復 MCP 伺服器分頁工具只返回首頁、圖像格式相容問題；減少虛假工具錯誤並改進終端渲染穩定性；提升 SDK 和無頭環境啟動速度（最快 2s）。該版本為維護型更新，重點聚焦穩定性提升。
...
**原文：** [claude-code-releases](https://github.com/anthropics/claude-code/releases/tag/v2.1.144)
