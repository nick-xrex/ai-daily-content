...
id: inbox_f56913ed
date: 2026-07-03
source_ref: "[[00-inbox/2026-07-03/0115-gitnexus-releases-rc-e46b87f29166338b63855e7dd3bcb9380abb8-494e]]"
title: "rc/e46b87f29166338b63855e7dd3bcb9380abb838f: feat: flat workspace index follows the checked-out branch (#2364)"
...
fetched_at: 2026-07-04T01:20:23.369170+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus 優化了 flat workspace index 的分支管理邏輯。Plain gitnexus analyze 現在總是針對 flat workspace slot，在分支切換時增量更新（而非自動路由到 branches/<slug>/ 子索引）。分支標籤在同一提交的乾淨樹切換時快速更新，避免了磁盤膨脹問題。影子清理機制在 flat slot 採用某標籤時，自動刪除原有的 branches/<slug>/ 子索引目錄及其註冊表摘要。MCP applyBranchScope 改進為優先查詢磁盤上的 flat meta，支持長期運行服務器解析新標記的分支。Status 命令改進為跳轉到 workspace index 後備方案，不再報告「未索引」死局。"
key_points:
  - "Plain analyze 總是針對 flat workspace slot，分支切換時增量更新，避免磁盤膨脹"
  - "影子清理：flat slot 採用標籤時刪除 branches/<slug>/ 子索引，自動磁盤管理"
  - "MCP 和 status 改進：flat meta 優先級提高、長期服務器支持、「未索引」錯誤改為 workspace index 後備方案"
tags: [gitnexus, workspace-index, branch-management]
topics: []
importance: 3
novelty: 2
insight_quality: 3
...
GitNexus 優化了 flat workspace index 的分支管理邏輯。Plain gitnexus analyze 現在總是針對 flat workspace slot，在分支切換時增量更新（而非自動路由到 branches/<slug>/ 子索引）。分支標籤在同一提交的乾淨樹切換時快速更新，避免了磁盤膨脹問題。影子清理機制在 flat slot 採用某標籤時，自動刪除原有的 branches/<slug>/ 子索引目錄及其註冊表摘要。MCP applyBranchScope 改進為優先查詢磁盤上的 flat meta，支持長期運行服務器解析新標記的分支。Status 命令改進為跳轉到 workspace index 後備方案，不再報告「未索引」死局。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/rc%2Fe46b87f29166338b63855e7dd3bcb9380abb838f)
