...
id: inbox_4120b419
date: 2026-06-21
source_ref: "[[00-inbox/2026-06-21/0106-simon-willison-sqlite-utils-4-0rc1-adds-migrations-and-2b65]]"
...
fetched_at: 2026-06-22T01:11:32.139556+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "sqlite-utils 4.0rc1 發布，Python 版本支援升級至 3.13，移除 3.8 支援。新增 migrations 功能：基於 decorator 的遷移系統，支援 Python 和 CLI 兩種執行方式，無逆向遷移支援（錯誤通過新遷移修復）。新增 db.atomic() 上下文管理器：利用 SQLite savepoints 實現原生 nested transactions，API 借鑑 Django/Peewee。包含多項 breaking changes：upsert 改用 INSERT...ON CONFLICT SET、FLOAT 改為 REAL、表列名改用雙引號、type detection 成為預設行為。CLI 工具現經由 sqlite-utils-tui 獨立插件提供。"
key_points:
  - "Migrations：decorator-based 系統，執行 `sqlite-utils migrate creatures.db migrations.py` 命令應用遷移，錯誤通過新遷移修復而非逆向"
  - "db.atomic() nested transactions：利用 SQLite savepoints，支援 try/except 內的巢狀 savepoint"
  - "Breaking changes：FLOAT→REAL、INSERT...ON CONFLICT SET 語法改變、type detection 預設啟用、支援 Python 3.13、移除 3.8"
tags: [sqlite-utils, migrations, transactions, database-tooling, python]
topics: []
importance: 3
novelty: 3
insight_quality: 4
...
**原文：** [simon-willison](https://simonwillison.net/2026/Jun/21/sqlite-utils-40rc1/#atom-everything)
