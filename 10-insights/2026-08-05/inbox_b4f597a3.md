...
id: inbox_b4f597a3
date: 2026-08-05
source_ref: "[[00-inbox/.../inbox_b4f597a3]]"
...
published_at: 2026-08-05T19:42:38+00:00
fetched_at: 2026-08-06T00:23:47.486079+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Simon Willison 用 Claude Fable 5(在 Claude Code for web 上運行)成功一鏡到底構建完整 Raccoon Heist 3D 遊戲，原始需求來自 2024 年一則 GPT-3 生成的遊戲概念推文。核心模式：用「work independently - do not ask me to make any further design decisions」授權 Claude 自主決策設計，搭配高頻 commit/push 循環提供預覽反饋。Claude 獨立選定 Three.js、用 gpt-image-2 生成紋理、用 Playwright 截圖自測(桌面+行動寬度)、發現行動端浣熊不可見後自行修復，產生標題頁，最後自主加入守衛犬 AI(視線無關的嗅覺追蹤系統、12 單位內追蹤、17 單位距離逃脫)。驗證路徑採自動化測試。完整遊戲可玩。"
key_points:
  - "Claude 自主決策模式：「獨立作業 + 不諮詢設計決策」的提示詞 + 高頻預覽迭代 = 完整功能交付"
  - "Claude 能力整合：Three.js + OpenAI API (gpt-image-2) + Playwright 自測 + GitHub 版本管理的無縫協作"
  - "自主功能擴張：Claude 未經指示自行加入守衛犬 AI(嗅覺追蹤、動態遊走、自動化測試驗證)"
tags: [claude, code-generation, autonomy, three-js, claude-code]
...
Simon Willison 用 Claude Fable 5(在 Claude Code for web 上運行)成功一鏡到底構建完整 Raccoon Heist 3D 遊戲，原始需求來自 2024 年一則 GPT-3 生成的遊戲概念推文。核心模式：用「work independently - do not ask me to make any further design decisions」授權 Claude 自主決策設計，搭配高頻 commit/push 循環提供預覽反饋。Claude 獨立選定 Three.js、用 gpt-image-2 生成紋理、用 Playwright 截圖自測(桌面+行動寬度)、發現行動端浣熊不可見後自行修復，產生標題頁，最後自主加入守衛犬 AI(視線無關的嗅覺追蹤系統、12 單位內追蹤、17 單位距離逃脫)。驗證路徑採自動化測試。完整遊戲可玩。
...
**原文：** [simon-willison](https://simonwillison.net/2026/Aug/5/raccoon-heist/#atom-everything)
...
Then it built out the first basic version of the game, then decided to "smoke-test in the pre-installed Chromium" using Playwright. This meant it could take screenshots of its own work and eyeball them . It did that for both desktop and mobile widths of the page, then noticed that the raccoon was invisible at mobile widths, so it fixed that : 
 ...
 How it's built: Vendored Three.js (no CDN), procedural low-poly characters, and a procedural WebAudio soundtrack (sneaky walking-bass jazz plus all sound effects — zero audio files). All seven ground/wall textures and the title-screen hero art were generated with gpt-image-2 and committed as static assets, so the deployed game makes no API calls
