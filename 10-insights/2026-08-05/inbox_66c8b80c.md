...
id: inbox_66c8b80c
date: 2026-08-05
source_ref: "[[00-inbox/.../inbox_66c8b80c]]"
title: "Release Candidate v1.6.10-rc.163"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.163
source: gitnexus-releases
published_at: 2026-08-05T06:11:28+00:00
fetched_at: 2026-08-06T00:20:37.770376+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.10-rc.163 是主分支自動化構建的 RC 版本，changelog 與後續三個 RC（rc.164–rc.166）完全相同。核心特性涵蓋 Docker 容器化支持、HTTP embedding backend（支自託管/遠端推理端點）、完整 Swift/iOS 語言支持（SPM 導入解析、建構子判別、export detection）、git commit 後自動重新索引保留 embeddings、新增 index CLI 命令。性能優化消除 Go 分析中的 O(n²) scope-capture re-walk，大型倉庫實現 parse cache 分片持久化。修復 MCP 啟動相容性、KuzuDB fork crash、全球升級 ENOTEMPTY 錯誤。資料庫遷移 KuzuDB → LadybugDB v0.15，加入 macOS 跨平台測試。"
key_points:
...
## Release Candidate v1.6.10-rc.163

GitNexus v1.6.10-rc.163 是主分支自動化構建的 RC 版本，changelog 與後續三個 RC（rc.164–rc.166）完全相同。核心特性涵蓋 Docker 容器化支持、HTTP embedding backend（支自託管/遠端推理端點）、完整 Swift/iOS 語言支持（SPM 導入解析、建構子判別、export detection）、git commit 後自動重新索引保留 embeddings、新增 index CLI 命令。性能優化消除 Go 分析中的 O(n²) scope-capture re-walk，大型倉庫實現 parse cache 分片持久化。修復 MCP 啟動相容性、KuzuDB fork crash、全球升級 ENOTEMPTY 錯誤。資料庫遷移 KuzuDB → LadybugDB v0.15，加入 macOS 跨平台測試。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.163)
...
Automated release candidate build from main .\n\n npm: npm install gitnexus@rc \n Version: 1.6.10-rc.163 \n Target base: 1.6.10 (rc #163 )\n Source commit (main): a6a8aa7 \n Release commit (versioned tree): 3e8479c \n\nRelease candidates are pre-stable builds intended for early testing. Stable releases remain on the latest dist-tag. 
