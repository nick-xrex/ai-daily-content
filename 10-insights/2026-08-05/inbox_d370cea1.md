...
id: inbox_d370cea1
date: 2026-08-05
source_ref: "[[00-inbox/.../inbox_d370cea1]]"
title: "Release Candidate v1.6.10-rc.164"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.164
source: gitnexus-releases
published_at: 2026-08-05T10:53:57+00:00
fetched_at: 2026-08-06T00:20:37.765339+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.10-rc.164 是主分支自動化構建的 RC 版本，changelog 與 rc.163、rc.165、rc.166 相同。主要特性包括 Docker 容器化支持、HTTP embedding backend（支自託管/遠端推理端點）、完整 Swift/iOS 語言支持（SPM 導入解析、建構子判別、export detection）、git commit 後自動重新索引並保留 embeddings、新增 index CLI 命令。性能優化上消除 Go 分析中的 O(n²) scope-capture re-walk，大型倉庫實現 parse cache 分片持久化。修復 MCP 啟動相容性、KuzuDB fork crash、全球升級 ENOTEMPTY 錯誤。資料庫從 KuzuDB 遷移至 LadybugDB v0.15，加入 macOS 跨平台測試。"
key_points:
  - "Docker 容器化部署 + HTTP embedding 自託管端點支持"
  - "完整 Swift/iOS 語言支持（SPM 解析、建構子判別、export detection）"
  - "O(n²) scope-capture re-walk 消除 + 大型倉庫 parse cache 分片持久化"
tags: [gitnexus, code-analysis, docker, embeddings, mcp]
topics: [agents.mcp]
importance: 3
novelty: 1
insight_quality: 2
...
## Release Candidate v1.6.10-rc.164

GitNexus v1.6.10-rc.164 是主分支自動化構建的 RC 版本，changelog 與 rc.163、rc.165、rc.166 相同。主要特性包括 Docker 容器化支持、HTTP embedding backend（支自託管/遠端推理端點）、完整 Swift/iOS 語言支持（SPM 導入解析、建構子判別、export detection）、git commit 後自動重新索引並保留 embeddings、新增 index CLI 命令。性能優化上消除 Go 分析中的 O(n²) scope-capture re-walk，大型倉庫實現 parse cache 分片持久化。修復 MCP 啟動相容性、KuzuDB fork crash、全球升級 ENOTEMPTY 錯誤。資料庫從 KuzuDB 遷移至 LadybugDB v0.15，加入 macOS 跨平台測試。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.164)
...
Automated release candidate build from main .\n\n npm: npm install gitnexus@rc \n Version: 1.6.10-rc.164 \n Target base: 1.6.10 (rc #164 )\n Source commit (main): 9372b17 \n Release commit (versioned tree): dc68094 \n\nRelease candidates are pre-stable builds intended for early testing. Stable releases remain on the latest dist-tag. 
