...
id: inbox_4578457e
date: 2026-08-05
source_ref: "[[00-inbox/.../inbox_4578457e]]"
title: "Release Candidate v1.6.10-rc.165"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.165
source: gitnexus-releases
published_at: 2026-08-05T11:38:02+00:00
fetched_at: 2026-08-06T00:20:37.761620+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.10-rc.165 是主分支自動化構建的 RC 版本，changelog 與 rc.164、rc.166 完全相同。核心特性涵蓋 Docker 容器化支持、HTTP embedding backend（支自託管/遠端推理端點）、完整 Swift/iOS 語言支持（SPM 導入解析、建構子判別、export detection）、git commit 自動重新索引保留 embeddings、新增 index CLI 命令。性能優化消除 Go 分析中 O(n²) scope-capture re-walk，實現大型倉庫 parse cache 分片持久化。修復 MCP 啟動相容性、KuzuDB fork crash、全球升級 ENOTEMPTY 錯誤。資料庫遷移 KuzuDB → LadybugDB v0.15，macOS 跨平台測試覆蓋。"
key_points:
...
GitNexus v1.6.10-rc.165 是主分支自動化構建的 RC 版本，changelog 與 rc.164、rc.166 完全相同。核心特性涵蓋 Docker 容器化支持、HTTP embedding backend（支自託管/遠端推理端點）、完整 Swift/iOS 語言支持（SPM 導入解析、建構子判別、export detection）、git commit 自動重新索引保留 embeddings、新增 index CLI 命令。性能優化消除 Go 分析中 O(n²) scope-capture re-walk，實現大型倉庫 parse cache 分片持久化。修復 MCP 啟動相容性、KuzuDB fork crash、全球升級 ENOTEMPTY 錯誤。資料庫遷移 KuzuDB → LadybugDB v0.15，macOS 跨平台測試覆蓋。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.165)
