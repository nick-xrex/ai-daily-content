...
id: inbox_89203bda
date: 2026-06-02
source_ref: "[[00-inbox/2026-06-02/0030-ruflo-releases-v3-10-33-ci-router-onnx-bug-cluster-d863]]"
title: "v3.10.33 — CI/router/ONNX bug cluster"
url: https://github.com/ruvnet/ruflo/releases/tag/v3.10.33
source: ruflo-releases
published_at: 2026-06-02T08:00:19+00:00
fetched_at: 2026-06-03T00:38:27.602818+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Ruflo v3.10.33 修復五個影響 CI、CLI、MCP 通訊和代理路由的高優先級缺陷。YAML 解析器未正確處理未加引號的冒號導致 v3-ci.yml 五次連續失敗，CI 系統無法產生 53 個應有的任務。--version 命令因過度啟動導致下載 23 MB ONNX 模型而阻塞 60–90 秒，在 npx 和 MCP stdio 30 秒逾時限制下觸發 SIGTERM。MCP stdio 寫入受 ruvector 和 ruvector-onnx-embeddings-wasm 庫的控制台輸出污染，破壞 JSON-RPC 幀；修復透過日誌過濾將 console.log 重導向 console.error。路由器的未錨定正則表達式在 11 個報告案例中誤分類關鍵字（例如 cd/ci/ui 出現在 decision/infrastructure 內），信心度從 0.8 降至 0.6。新增靜態迴歸防護指令碼在 CI 早期階段檢驗 YAML 語法和路由正則。"
key_points:
  - "--version 阻塞時間從 60–90 秒降至 33 毫秒，通過在 CLI 啟動前從 package.json 讀取版本、避免 23 MB ONNX 模型下載"
  - "YAML 冒號未加引號導致 v3-ci.yml 無法產生 53 個 CI 任務；新增 smoke-workflows-yaml.mjs 脚本防止未來重複"
  - "MCP stdio JSON-RPC 幀污染由上游庫的 console.log 進度訊息引起；log-filters.ts 將已知前綴重導向 stderr，confirmed 端對端測試通過"
tags: [ruflo, ci-cd, bug-fix, mcp-integration, performance]
topics: []
importance: 4
novelty: 2
insight_quality: 4
...
Ruflo v3.10.33 修復五個影響 CI、CLI、MCP 通訊和代理路由的高優先級缺陷。YAML 解析器未正確處理未加引號的冒號導致 v3-ci.yml 五次連續失敗，CI 系統無法產生 53 個應有的任務。--version 命令因過度啟動導致下載 23 MB ONNX 模型而阻塞 60–90 秒，在 npx 和 MCP stdio 30 秒逾時限制下觸發 SIGTERM。MCP stdio 寫入受 ruvector 和 ruvector-onnx-embeddings-wasm 庫的控制台輸出污染，破壞 JSON-RPC 幀；修復透過日誌過濾將 console.log 重導向 console.error。路由器的未錨定正則表達式在 11 個報告案例中誤分類關鍵字（例如 cd/ci/ui 出現在 decision/infrastructure 內），信心度從 0.8 降至 0.6。新增靜態迴歸防護指令碼在 CI 早期階段檢驗 YAML 語法和路由正則。
...
**原文：** [ruflo-releases](https://github.com/ruvnet/ruflo/releases/tag/v3.10.33)
