...
id: inbox_230f43f1
date: 2026-06-02
source_ref: "[[00-inbox/2026-06-02/0030-gitnexus-releases-release-candidate-v1-6-6-rc-114-a818]]"
title: "Release Candidate v1.6.6-rc.114"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.6-rc.114
source: gitnexus-releases
published_at: 2026-06-02T07:46:27+00:00
fetched_at: 2026-06-03T00:36:32.118902+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.6-rc.114 發布。本版本為自動化 RC 構建，包含：(1) MCP 啟動相容性與 KuzuDB fork 崩潰修復；(2) Git 提交後自動重建索引（保留嵌入向量）、HTTP 嵌入後端（支持自託管與遠端端點）、完整 Swift 支持、Docker 支持等新功能；(3) 全局升級時 ENOTEMPTY 錯誤修復、使用者可寫快取目錄設定、大倉庫解析快取分片等 bug 修復；(4) Go scope-capture O(n²) 重走問題解決；(5) 跨平台測試矩陣擴展至 macOS。RC 版本使用未穩定 dist-tag，穩定版本保持不變。"
key_points:
  - "自動重建索引 (auto-reindex after git commit) 保留嵌入向量，減少重新計算成本"
  - "HTTP 嵌入後端支持自託管與遠端端點，解耦計算資源與主應用"
  - "Go scope-capture O(n²) 重走問題修復，提升大型倉庫解析效能"
tags: [gitnexus, code-intelligence, release-candidate, performance-fix]
topics: []
importance: 3
novelty: 2
insight_quality: 2
...
## Release Candidate v1.6.6-rc.114

GitNexus v1.6.6-rc.114 發布。本版本為自動化 RC 構建，包含：(1) MCP 啟動相容性與 KuzuDB fork 崩潰修復；(2) Git 提交後自動重建索引（保留嵌入向量）、HTTP 嵌入後端（支持自託管與遠端端點）、完整 Swift 支持、Docker 支持等新功能；(3) 全局升級時 ENOTEMPTY 錯誤修復、使用者可寫快取目錄設定、大倉庫解析快取分片等 bug 修復；(4) Go scope-capture O(n²) 重走問題解決；(5) 跨平台測試矩陣擴展至 macOS。RC 版本使用未穩定 dist-tag，穩定版本保持不變。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.6-rc.114)
...
Automated release candidate build from main .\n\n npm: npm install gitnexus@rc \n Version: 1.6.6-rc.114 \n Target base: 1.6.6 (rc #114 )\n Source commit (main): fcddbb0 \n Release commit (versioned tree): ed100cf \n\nRelease candidates are pre-stable builds intended for early testing. Stable releases remain on the latest dist-tag. 
