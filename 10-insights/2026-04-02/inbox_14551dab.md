---
id: inbox_14551dab
date: 2026-04-02
source_ref: "[[00-inbox/2026-04-02/0158-openai-blog-codex-now-offers-more-flexible-pricing-f-731e]]"
title: "Codex now offers more flexible pricing for teams"
url: https://openai.com/index/codex-flexible-pricing-for-teams
source: openai-blog
published_at: 2026-04-02T10:00:00+00:00
fetched_at: 2026-04-21T02:08:10.091349+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "OpenAI 為 Codex 推出按使用量計費（pay-as-you-go）的定價模式。此新定價適用於 ChatGPT Business 和 Enterprise 方案。該模式降低了團隊的初期成本和採用障礙。企業現在可以更靈活地開始試用和擴展 Codex 的使用規模。此舉有助於加速企業級 AI 編碼工具的采用。"
key_points:
  - "Codex 推出 pay-as-you-go 按使用量計費"
  - "支持 ChatGPT Business 和 Enterprise 方案"
  - "降低企業初期成本和採用門檻"
tags: [codex, pricing-model, enterprise-ai]
topics: [foundation_models.gpt]
importance: 3
novelty: 3
deep_dive_candidate: false
deep_dive_approved: false
---

## Codex now offers more flexible pricing for teams

OpenAI 為 Codex 推出按使用量計費（pay-as-you-go）的定價模式。此新定價適用於 ChatGPT Business 和 Enterprise 方案。該模式降低了團隊的初期成本和採用障礙。企業現在可以更靈活地開始試用和擴展 Codex 的使用規模。此舉有助於加速企業級 AI 編碼工具的采用。

### 重點
- Codex 推出 pay-as-you-go 按使用量計費
- 支持 ChatGPT Business 和 Enterprise 方案
- 降低企業初期成本和採用門檻

**原文：** [openai-blog](https://openai.com/index/codex-flexible-pricing-for-teams)

---

### 📄 原文內容

<details>
<summary>點此展開 / 收合</summary>

Codex now includes pay-as-you-go pricing for ChatGPT Business and Enterprise, providing teams a more flexible option to start and scale adoption.

</details>
