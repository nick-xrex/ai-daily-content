...
id: inbox_cf352e9b
date: 2026-04-23
source_ref: "[[00-inbox/2026-04-23/0246-simon-willison-extract-pdf-text-in-your-browser-with-li-2eda]]"
...
published_at: 2026-04-23T21:54:24+00:00
fetched_at: 2026-04-24T02:56:27.830730+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Simon Willison 將 LlamaIndex 的 LiteParse 工具（PDF 文字提取）改寫為瀏覽器版本，完全在客戶端運行無需上傳檔案。LiteParse 核心能力是「空間文字解析」：使用啟發式方法自動偵測多欄排版、表格、標題位置等，按邏輯順序而非座標順序輸出文字。工作流程展示 Claude Code + Opus 4.7 的最佳實踐：先撰寫 plan.md 規劃、使用紅綠 TDD、小步提交、接續迭代。最終網頁版支援有/無 OCR 模式、JSON 格式輸出、複製按鈕、行動友善設計。使用 PDF.js 和 Tesseract.js 完全在瀏覽器中運行。"
key_points:
  - "瀏覽器版 LiteParse 使用 PDF.js + Tesseract.js，檔案留在本機無網傳"
  - "空間文字解析自動處理複雜排版，優於座標順序提取"
  - "示範 Claude Code 工作流：規劃 → TDD →增量迭代，可參考 GitHub plan.md"
tags: [pdf-parsing, liteparse, claude-code, browser-tooling, tdd]
topics: []
importance: 3
novelty: 4
insight_quality: 4
...
Simon Willison 將 LlamaIndex 的 LiteParse 工具（PDF 文字提取）改寫為瀏覽器版本，完全在客戶端運行無需上傳檔案。LiteParse 核心能力是「空間文字解析」：使用啟發式方法自動偵測多欄排版、表格、標題位置等，按邏輯順序而非座標順序輸出文字。工作流程展示 Claude Code + Opus 4.7 的最佳實踐：先撰寫 plan.md 規劃、使用紅綠 TDD、小步提交、接續迭代。最終網頁版支援有/無 OCR 模式、JSON 格式輸出、複製按鈕、行動友善設計。使用 PDF.js 和 Tesseract.js 完全在瀏覽器中運行。
...
**原文：** [simon-willison](https://simonwillison.net/2026/Apr/23/liteparse-for-the-web/#atom-everything)
