...
id: inbox_a9154577
date: 2026-08-07
source_ref: "[[00-inbox/.../inbox_a9154577]]"
title: "Release Candidate v1.6.10-rc.174"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.174
source: gitnexus-releases
published_at: 2026-08-07T11:24:26+00:00
fetched_at: 2026-08-11T01:17:01.043543+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.10-rc.174 發布重大性能與基礎設施改進：Go 解析器消除 O(n²) 作用域捕捉重複遍歷（優化至 O(n)），新增 HTTP 嵌入式後端支援自託管/遠端端點部署，完整 Swift 語言支援（查詢修正、匯出偵測、隱含匯入、建構函式解析），Docker 容器支援，以及自動重新索引保留嵌入式資料。修復 npm 全域升級 ENOTEMPTY 錯誤、快取持久化分片、MCP 伺服器並列工具呼叫崩潰。資料庫從 KuzuDB 遷移至 LadybugDB v0.15。"
key_points:
  - "GitNexus Go 解析器優化 O(n²) → O(n) 作用域捕捉重複遍歷（PR#1915），顯著降低大型儲存庫索引時間"
  - "新增 HTTP 嵌入式後端支援自託管與遠端嵌入式端點部署，解耦嵌入式基礎設施（PR#395）"
  - "資料庫遷移 KuzuDB → LadybugDB v0.15 + Docker 容器化 + 自動重新索引保留嵌入資料（PR#205）"
tags: [performance-optimization, go-parser, embeddings-backend, docker-support, database-migration]
topics: [agents.mcp]
importance: 4
novelty: 4
insight_quality: 4
...
GitNexus v1.6.10-rc.174 發布重大性能與基礎設施改進：Go 解析器消除 O(n²) 作用域捕捉重複遍歷（優化至 O(n)），新增 HTTP 嵌入式後端支援自託管/遠端端點部署，完整 Swift 語言支援（查詢修正、匯出偵測、隱含匯入、建構函式解析），Docker 容器支援，以及自動重新索引保留嵌入式資料。修復 npm 全域升級 ENOTEMPTY 錯誤、快取持久化分片、MCP 伺服器並列工具呼叫崩潰。資料庫從 KuzuDB 遷移至 LadybugDB v0.15。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.174)
