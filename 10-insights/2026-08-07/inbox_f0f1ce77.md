...
id: inbox_f0f1ce77
date: 2026-08-07
source_ref: "[[00-inbox/.../inbox_f0f1ce77]]"
title: "Release Candidate v1.6.10-rc.173"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.173
source: gitnexus-releases
published_at: 2026-08-07T09:19:15+00:00
fetched_at: 2026-08-11T01:19:12.664140+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.10-rc.173 於 2026-08-07 發布，涵蓋多項功能與最佳化。關鍵改進包括：完整 Swift 語言支援（含查詢修復、匯出偵測、隱式 imports、建構函式解析），HTTP embedding backend 支援自主託管及遠端端點，Docker 容器化支援，git commit 後自動重新索引同時保留 embeddings。效能改進方面，Go 分析器消除了 O(n²) 的 scope-capture 重複走訪，改為 O(n)（#1915），對大型倉庫有顯著影響。安全性與穩定性修復包括 MCP startup 相容性改進、MCP server 並行工具呼叫崩潰修復（#326）、全域升級的 ENOTEMPTY 錯誤修復（#843）。語言覆蓋從 6 個擴展到 10+（新增 Kotlin、Ruby、PHP 8+/Laravel），資料庫從 KuzuDB 遷移至 LadybugDB v0.15。"
key_points:
  - "完整 Swift 語言支援（查詢、匯出、隱式 imports、建構函式）；新增 Kotlin、Ruby、PHP 8+/Laravel；覆蓋 10+ 語言"
  - "O(n²) → O(n) Go 分析器最佳化（#1915 消除 scope-capture 重複走訪）；parse cache sharding 用於大型倉庫（#1580）"
  - "HTTP embedding backend（#395 自主託管/遠端）；Docker 支援（#848）；git commit 自動重索引保留 embeddings（#205）；MCP 並行工具呼叫崩潰修復（#326）"
tags: [gitnexus, language-support, performance-optimization, embeddings-backend, docker-support]
topics: []
importance: 4
novelty: 3
insight_quality: 4
...
## Release Candidate v1.6.10-rc.173

GitNexus v1.6.10-rc.173 於 2026-08-07 發布，涵蓋多項功能與最佳化。關鍵改進包括：完整 Swift 語言支援（含查詢修復、匯出偵測、隱式 imports、建構函式解析），HTTP embedding backend 支援自主託管及遠端端點，Docker 容器化支援，git commit 後自動重新索引同時保留 embeddings。效能改進方面，Go 分析器消除了 O(n²) 的 scope-capture 重複走訪，改為 O(n)（#1915），對大型倉庫有顯著影響。安全性與穩定性修復包括 MCP startup 相容性改進、MCP server 並行工具呼叫崩潰修復（#326）、全域升級的 ENOTEMPTY 錯誤修復（#843）。語言覆蓋從 6 個擴展到 10+（新增 Kotlin、Ruby、PHP 8+/Laravel），資料庫從 KuzuDB 遷移至 LadybugDB v0.15。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.173)
...
Automated release candidate build from main .\n\n npm: npm install gitnexus@rc \n Version: 1.6.10-rc.173 \n Target base: 1.6.10 (rc #173 )\n Source commit (main): e69d3c4 \n Release commit (versioned tree): 9faa1ef \n\nRelease candidates are pre-stable builds intended for early testing. Stable releases remain on the latest dist-tag. 
