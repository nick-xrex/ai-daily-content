---
id: inbox_be738352
date: 2026-04-09
source_ref: "[[00-inbox/2026-04-09/0158-openai-blog-cyberagent-moves-faster-with-chatgpt-ent-4b6d]]"
title: "CyberAgent moves faster with ChatGPT Enterprise and Codex"
url: https://openai.com/index/cyberagent
source: openai-blog
published_at: 2026-04-09T00:00:00+00:00
fetched_at: 2026-04-21T02:07:11.286913+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "廣告科技公司 CyberAgent 採用 OpenAI 的 ChatGPT Enterprise 和 Codex，在廣告、媒體和遊戲領域實現 AI 的安全規模化。ChatGPT Enterprise 提供企業級加密和治理，Codex 驅動廣告文案優化、媒體策劃和遊戲開發工作。該舉措加速內部決策流程，實現跨部門 AI 標準化和品質一致性。"
key_points:
  - "CyberAgent 使用 ChatGPT Enterprise 的企業加密和治理功能進行安全規模化部署"
  - "Codex 支援廣告文案、媒體策劃、遊戲開發三大工作流"
  - "實現跨廣告、媒體、遊戲部門的 AI 標準化和品質提升"
tags: [enterprise-ai, chatgpt-enterprise, codex, case-study]
topics: [foundation_models.gpt]
importance: 3
novelty: 2
deep_dive_candidate: false
deep_dive_approved: false
---

## CyberAgent moves faster with ChatGPT Enterprise and Codex

廣告科技公司 CyberAgent 採用 OpenAI 的 ChatGPT Enterprise 和 Codex，在廣告、媒體和遊戲領域實現 AI 的安全規模化。ChatGPT Enterprise 提供企業級加密和治理，Codex 驅動廣告文案優化、媒體策劃和遊戲開發工作。該舉措加速內部決策流程，實現跨部門 AI 標準化和品質一致性。

### 重點
- CyberAgent 使用 ChatGPT Enterprise 的企業加密和治理功能進行安全規模化部署
- Codex 支援廣告文案、媒體策劃、遊戲開發三大工作流
- 實現跨廣告、媒體、遊戲部門的 AI 標準化和品質提升

**原文：** [openai-blog](https://openai.com/index/cyberagent)

---

### 📄 原文內容

<details>
<summary>點此展開 / 收合</summary>

CyberAgent uses ChatGPT Enterprise and Codex to securely scale AI adoption, improve quality, and accelerate decisions across advertising, media, and gaming.

</details>
