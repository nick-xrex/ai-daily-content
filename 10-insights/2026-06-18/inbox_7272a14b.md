...
id: inbox_7272a14b
date: 2026-06-18
source_ref: "[[00-inbox/2026-06-18/2200-simon-willison-datasette-apps-host-custom-html-applicat-ced5]]"
...
fetched_at: 2026-06-19T22:10:15.799096+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Simon Willison 推出 datasette-apps 插件，讓用戶在 Datasette 內建立沙箱化的 HTML+JavaScript 應用。應用在受限 <iframe sandbox> 內運行，配合 CSP（Content Security Policy）防止 cookie 盜取和外部資料外洩，且 CSP 策略設置後不可被內部 JavaScript 修改。核心通訊機制使用 MessageChannel() 而非 postMessage()，實現更安全的父窗口-子窗口協議，頁面導航時通道自動關閉。應用可執行唯讀 SQL 查詢，也可通過 datasette stored queries 進行條件寫入（insert/update）。內建 SQL 日誌和錯誤追蹤，支持基於失敗情況動態構建 CSP 白名單。插件對 LLM 友善，自動生成可複製的 prompt（含數據庫 schema），用戶可直接粘貼給 Claude、GPT 或 Gemini 生成應用代碼。此概念源自 Claude Artifacts，已推廣為 Datasette 生態的獨立功能。"
key_points:
  - "iframe sandbox + CSP 組合：<iframe sandbox=\"allow-scripts\"> 配合不可改寫的 <meta http-equiv=\"Content-Security-Policy\"> header，實現無法訪問 cookies/localStorage 且無法連接外部域的隔離"
  - "MessageChannel() 安全通訊：應用透過 MessageChannel() 而非 postMessage() 與父窗口通訊，頁面導航時自動關閉通道，防止未信任外部頁面的命令執行，體現「防禦縱深」原則"
  - "Stored queries 條件寫入：應用通過 datasette.storedQuery(dbname, queryname, params) API 調用白名單內的寫入查詢，實現細粒度的插入/更新控制"
tags: [datasette, iframe-sandbox, csp-security, stored-queries, llm-code-generation]
topics: []
importance: 4
novelty: 4
insight_quality: 4
...
**原文：** [simon-willison](https://simonwillison.net/2026/Jun/18/datasette-apps/#atom-everything)
