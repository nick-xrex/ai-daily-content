...
id: inbox_15d5f680
date: 2026-06-18
source_ref: "[[00-inbox/2026-06-18/2200-gitnexus-releases-release-candidate-v1-6-8-rc-54-dcbb]]"
title: "Release Candidate v1.6.8-rc.54"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.8-rc.54
source: gitnexus-releases
published_at: 2026-06-18T21:14:42+00:00
fetched_at: 2026-06-18T22:04:31.717697+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus 發布 v1.6.8-rc.54（釋出候選版本），是面向代碼智慧的重大擴展版本。新增特性包括 Docker 支援、HTTP embedding backend（支援自託管和遠端端點）、Swift、Ruby、Kotlin、PHP 8+/Laravel 的完整語言支援，以及 DeepSeek 模型配置。安全方面改善 MCP 啟動相容性和 CLI 指令延遲載入。性能優化消除 Go 語言 scope-capture 的 O(n²) re-walks（解決 #1848 隔離）、大型 repo 的 parse cache 分片持久化。Bug fixes 涵蓋全域升級的 ENOTEMPTY 錯誤、緩存目錄權限、tree-sitter-proto 安裝等問題。"
key_points:
  - "新增 Docker 支援、HTTP embedding backend、Swift/Ruby/Kotlin/PHP 8+ 完整語言支援，支援 DeepSeek 模型配置"
  - "消除 Go scope-capture O(n²) 複雜度 re-walks，大型 repo parse cache 分片持久化，減少記憶體開銷"
  - "改善 MCP 啟動相容性、CLI 延遲載入、預安裝清理防止 ENOTEMPTY、權限修復，增強全域升級穩定性"
tags: [gitnexus, code-intelligence, multi-language, performance-optimization, docker]
topics: []
importance: 2
novelty: 3
insight_quality: 4
...
GitNexus 發布 v1.6.8-rc.54（釋出候選版本），是面向代碼智慧的重大擴展版本。新增特性包括 Docker 支援、HTTP embedding backend（支援自託管和遠端端點）、Swift、Ruby、Kotlin、PHP 8+/Laravel 的完整語言支援，以及 DeepSeek 模型配置。安全方面改善 MCP 啟動相容性和 CLI 指令延遲載入。性能優化消除 Go 語言 scope-capture 的 O(n²) re-walks（解決 #1848 隔離）、大型 repo 的 parse cache 分片持久化。Bug fixes 涵蓋全域升級的 ENOTEMPTY 錯誤、緩存目錄權限、tree-sitter-proto 安裝等問題。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.8-rc.54)
