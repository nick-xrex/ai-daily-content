...
id: inbox_aa1f835d
date: 2026-06-18
source_ref: "[[00-inbox/2026-06-18/2200-codex-releases-0-141-0-d575]]"
title: "0.141.0"
url: https://github.com/openai/codex/releases/tag/rust-v0.141.0
source: codex-releases
published_at: 2026-06-18T04:44:21+00:00
fetched_at: 2026-06-18T22:04:31.711231+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "OpenAI Codex 發布 rust-v0.141.0，重點聚焦遠端執行、MCP 伺服器整合、跨平台相容性和性能最佳化。遠端執行器現使用認證端到端加密 Noise relay channels；跨平台遠端執行保留執行器原生工作目錄和殼層；選定執行器插件支援 stdio MCP 伺服器動態激活。修復超過 20 項 bug，包括 Windows 沙箱憑證、TLS P-521 簽章、idle relay 連線、hook 信任、plugin 能力路由等問題。性能優化減少工具搜索/歷史複製/WebSocket 序列化開銷；限制提示圖像緩存至 64 MiB、反饋上傳至 8 個相關執行緒。"
key_points:
  - "遠端執行器採用認證端到端加密 Noise relay channels，提升安全隔離與完整性"
  - "跨平台遠端執行保留執行器原生工作目錄、環境變數、殼層，改善 Windows 和 Unix 相容性"
  - "工具搜索緩存、歷史複製消除、WebSocket 直接序列化、圖像緩存限制至 64 MiB，整體降低延遲和記憶體使用"
tags: [codex, mcp-servers, remote-execution, security-encryption]
...
OpenAI Codex 發布 rust-v0.141.0，重點聚焦遠端執行、MCP 伺服器整合、跨平台相容性和性能最佳化。遠端執行器現使用認證端到端加密 Noise relay channels；跨平台遠端執行保留執行器原生工作目錄和殼層；選定執行器插件支援 stdio MCP 伺服器動態激活。修復超過 20 項 bug，包括 Windows 沙箱憑證、TLS P-521 簽章、idle relay 連線、hook 信任、plugin 能力路由等問題。性能優化減少工具搜索/歷史複製/WebSocket 序列化開銷；限制提示圖像緩存至 64 MiB、反饋上傳至 8 個相關執行緒。
...
**原文：** [codex-releases](https://github.com/openai/codex/releases/tag/rust-v0.141.0)
