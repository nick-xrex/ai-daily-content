...
id: inbox_40fec9f7
date: 2026-07-26
source_ref: "[[00-inbox/2026-07-26/0123-gitnexus-releases-release-candidate-v1-6-10-rc-112-4c85]]"
title: "Release Candidate v1.6.10-rc.112"
url: https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.112
source: gitnexus-releases
published_at: 2026-07-26T15:46:26+00:00
fetched_at: 2026-07-27T01:26:39.622806+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "GitNexus v1.6.10-rc.112 達成代碼智能重大升級：(1) Phase 8 型別解析完成欄位/屬性級別追蹤，新增 ACCESSES 邊類型記錄讀寫操作，10+ 語言覆蓋；(2) 消除 Go 解析器 O(n²) scope-capture re-walks，修正全域升級 ENOTEMPTY 錯誤、快取持久化問題；(3) 多語言擴展完成（Swift 含 SPM imports 與 constructor 辨別、Kotlin、Ruby、PHP 8+ Laravel），新增 Docker 支持與 HTTP 嵌入後端；(4) 資料庫從 KuzuDB 遷移至 LadybugDB v0.15；(5) CLI 新增 'index' 命令註冊現有 .gitnexus/ 資料夾，git commit 後自動重索引並保留嵌入。MCP 啟動相容性與 CLI 命令延遲載入強化。"
key_points:
  - "Phase 8 型別解析：欄位級別追蹤 + ACCESSES 邊（read/write 分類），10+ 語言完整支持"
  - "效能突破：消除 Go 解析器 O(n²) scope-capture re-walks；全域升級 ENOTEMPTY 修正；快取分片持久化修復"
  - "多語言完成（Swift SPM/constructor、Kotlin、Ruby、PHP Laravel）+ Docker + HTTP embedding + LadybugDB 遷移 + auto-reindex"
tags: [gitnexus, type-resolution-phase-8, code-intelligence, multi-language-support, performance-optimization, database-migration, docker-support]
topics: [agents.mcp]
importance: 4
novelty: 4
insight_quality: 4
...
## Release Candidate v1.6.10-rc.112

GitNexus v1.6.10-rc.112 達成代碼智能重大升級：(1) Phase 8 型別解析完成欄位/屬性級別追蹤，新增 ACCESSES 邊類型記錄讀寫操作，10+ 語言覆蓋；(2) 消除 Go 解析器 O(n²) scope-capture re-walks，修正全域升級 ENOTEMPTY 錯誤、快取持久化問題；(3) 多語言擴展完成（Swift 含 SPM imports 與 constructor 辨別、Kotlin、Ruby、PHP 8+ Laravel），新增 Docker 支持與 HTTP 嵌入後端；(4) 資料庫從 KuzuDB 遷移至 LadybugDB v0.15；(5) CLI 新增 'index' 命令註冊現有 .gitnexus/ 資料夾，git commit 後自動重索引並保留嵌入。MCP 啟動相容性與 CLI 命令延遲載入強化。
...
**原文：** [gitnexus-releases](https://github.com/abhigyanpatwari/GitNexus/releases/tag/v1.6.10-rc.112)
...
Automated release candidate build from main .\n\n npm: npm install gitnexus@rc \n Version: 1.6.10-rc.112 \n Target base: 1.6.10 (rc #112 )\n Source commit (main): 8307e3f \n Release commit (versioned tree): 4acbb08 \n\nRelease candidates are pre-stable builds intended for early testing. Stable releases remain on the latest dist-tag. 
