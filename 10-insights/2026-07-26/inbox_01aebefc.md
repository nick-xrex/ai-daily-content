...
id: inbox_01aebefc
date: 2026-07-26
source_ref: "[[00-inbox/2026-07-26/0123-rtk-releases-v0-44-0-32f4]]"
title: "v0.44.0"
url: https://github.com/rtk-ai/rtk/releases/tag/v0.44.0
source: rtk-releases
published_at: 2026-07-26T12:45:56+00:00
fetched_at: 2026-07-27T01:30:01.713944+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "RTK v0.44.0 是一次功能扩展发布，新增 git checkout、uv run、Kimi AI agent、Factory Droid 集成、SBT (Scala Build Tool) 支持等特性。重点是完整的 PHP 工具链统一（php, artisan, phpunit, phpstan, pest, paratest, ecs, pint 共 7 个工具一体化集成）和 TOML 过滤器链路完整化。bug 修复方面聚焦于 UTF-8 字符边界处理（修复 50+ 处截断问题，使用 byte offsets 替代 char indices），Cargo JSON 格式解析完整化，权限决策记录落地（hooks emit permissionDecision），以及自定义过滤器的信任管理机制（项目级 + 全局 opt-in 标志）。"
key_points:
  - "多语言支持扩展：新增 git checkout、uv run、Kimi AI agent、SBT (Scala)，PHP 工具链 7 工具一体化（artisan/phpunit/phpstan/pest/paratest/ecs/pint）"
  - "编码健壮性改进：UTF-8 字符边界修复覆盖 50+ 位置（byte offsets 替代 char indices），避免截断导致的数据损坏"
  - "安全与审计增强：自定义过滤器信任管理（项目 + 全局预设），权限决策记录，CLI 和 IDE 钩子差异化处理"
tags: [rtk, multi-language-support, tool-integration, utf8-handling, security-hardening]
topics: []
importance: 4
novelty: 3
insight_quality: 4
...
RTK v0.44.0 是一次功能扩展发布，新增 git checkout、uv run、Kimi AI agent、Factory Droid 集成、SBT (Scala Build Tool) 支持等特性。重点是完整的 PHP 工具链统一（php, artisan, phpunit, phpstan, pest, paratest, ecs, pint 共 7 个工具一体化集成）和 TOML 过滤器链路完整化。bug 修复方面聚焦于 UTF-8 字符边界处理（修复 50+ 处截断问题，使用 byte offsets 替代 char indices），Cargo JSON 格式解析完整化，权限决策记录落地（hooks emit permissionDecision），以及自定义过滤器的信任管理机制（项目级 + 全局 opt-in 标志）。
...
**原文：** [rtk-releases](https://github.com/rtk-ai/rtk/releases/tag/v0.44.0)
