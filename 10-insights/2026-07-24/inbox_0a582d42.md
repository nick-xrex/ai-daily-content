...
id: inbox_0a582d42
date: 2026-07-24
source_ref: "[[00-inbox/2026-07-24/0123-repowise-releases-v0-35-0-1147]]"
title: "v0.35.0"
...
fetched_at: 2026-07-27T01:35:26.361814+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Repowise v0.35.0 發佈大幅架構升級，核心轉變為概念優先的文檔導航，取代傳統檔案/目錄結構。主要創新包括：零成本 init --index-only 生成完整 wiki（無需 LLM 或 API key）、repowise generate 的成本可控 scoped 生成、結合全文搜尋與向量搜尋的 RRF（倒數排名融合）混合檢索。新增互動式變耦合分析頁面、死代碼檢測信心度量表、大規模 AI 文檔生成與溯源追蹤。支援 Codex 與 Claude plugin 特性對等，優化 TSX 語法回退與 Ollama 推理控制。整體聚焦於將程式碼分析從檔案維度升級至語意概念維度。"
key_points:
  - "引入概念驅動導航架構，頁面層級持久化於數據模型；zero-cost init --index-only 無需 LLM 即可生成完整 wiki，降低採用成本"
  - "Repowise generate 分類支持 scoped、cost-gated 生成，並新增 coverage picker 與互動式排名選擇器，讓用戶精細控制文檔生成範圍與成本"
  - "Search_codebase 採用 RRF 融合全文搜尋（FTS）與向量搜尋，提升精確度與召回率；新增 concept-tree position 在 MCP get_context 中曝露"
tags: [repowise, code-intelligence, wiki-generation, concept-first, rrf-fusion]
topics: []
importance: 4
novelty: 4
insight_quality: 4
...
Repowise v0.35.0 發佈大幅架構升級，核心轉變為概念優先的文檔導航，取代傳統檔案/目錄結構。主要創新包括：零成本 init --index-only 生成完整 wiki（無需 LLM 或 API key）、repowise generate 的成本可控 scoped 生成、結合全文搜尋與向量搜尋的 RRF（倒數排名融合）混合檢索。新增互動式變耦合分析頁面、死代碼檢測信心度量表、大規模 AI 文檔生成與溯源追蹤。支援 Codex 與 Claude plugin 特性對等，優化 TSX 語法回退與 Ollama 推理控制。整體聚焦於將程式碼分析從檔案維度升級至語意概念維度。
...
**原文：** [repowise-releases](https://github.com/repowise-dev/repowise/releases/tag/v0.35.0)
