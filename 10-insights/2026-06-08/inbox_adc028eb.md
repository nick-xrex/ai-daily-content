...
id: inbox_adc028eb
date: 2026-06-08
source_ref: "[[00-inbox/2026-06-08/2200-codex-releases-0-138-0-1814]]"
title: "0.138.0"
url: https://github.com/openai/codex/releases/tag/rust-v0.138.0
source: codex-releases
published_at: 2026-06-08T23:01:11+00:00
fetched_at: 2026-06-09T22:04:18.468216+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "OpenAI Codex 0.138.0 發布。新增 /app command，支持將當前 CLI thread handoff 到 macOS 和 Windows 原生 Codex Desktop（Windows workspace 啟動時可直接進入 Desktop）。Local image attachments 和 standalone image generations 現暴露保存的文件路徑，改善後續編輯可靠性。Reasoning effort selection 靈活化，支持模型定義的 effort level 順序。App-server integrations 可讀取帳戶 token 使用情況；Codex auth 支持 v2 personal access tokens。Plugin automation 結構化輸出豐富化，add/remove/marketplace commands 支持 --json。修復 goal workflows 多行粘貼提前提交、TUI UI 空白行、cancelled prompts 光標位置等問題。"
key_points:
  - "/app desktop handoff：CLI thread 可直接無縫切換到 macOS/Windows Codex Desktop"
  - "Image path exposure：Local images 和生成的圖片暴露文件路徑，改善 follow-up edits 可靠性"
  - "Reasoning effort 靈活化：支持 model-defined effort levels 和自定義順序"
tags: [codex, desktop-handoff, image-paths, reasoning-effort]
topics: []
importance: 4
novelty: 4
insight_quality: 3
...
OpenAI Codex 0.138.0 發布。新增 /app command，支持將當前 CLI thread handoff 到 macOS 和 Windows 原生 Codex Desktop（Windows workspace 啟動時可直接進入 Desktop）。Local image attachments 和 standalone image generations 現暴露保存的文件路徑，改善後續編輯可靠性。Reasoning effort selection 靈活化，支持模型定義的 effort level 順序。App-server integrations 可讀取帳戶 token 使用情況；Codex auth 支持 v2 personal access tokens。Plugin automation 結構化輸出豐富化，add/remove/marketplace commands 支持 --json。修復 goal workflows 多行粘貼提前提交、TUI UI 空白行、cancelled prompts 光標位置等問題。
...
**原文：** [codex-releases](https://github.com/openai/codex/releases/tag/rust-v0.138.0)
