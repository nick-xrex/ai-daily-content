...
id: inbox_45a5ffda
date: 2026-06-08
source_ref: "[[00-inbox/2026-06-08/2200-claude-code-releases-v2-1-169-b7bd]]"
title: "v2.1.169"
url: https://github.com/anthropics/claude-code/releases/tag/v2.1.169
source: claude-code-releases
published_at: 2026-06-08T21:57:18+00:00
fetched_at: 2026-06-09T22:04:18.461359+00:00
model: claude-haiku-4-5
tokens_in: 0
tokens_out: 0
summary_zh: "Claude Code v2.1.169 發布，新增三項關鍵功能：--safe-mode flag（禁用 CLAUDE.md、plugins、skills、hooks、MCP servers 以隔離故障），/cd command（在 session 中無損切換工作目錄並保留 prompt cache），disableBundledSkills 設定（隱藏內置 skills 和 slash commands）。修復 MCP 企業策略（allowedMcpServers/deniedMcpServers）在 reconnect、IDE 配置、首次啟動後未被正確執行的問題；修復 macOS 啟動時 ~30–50ms UI stall；修復 claude -p 在 Windows 上 slash-command 掃描緩慢的回歸。改進 TaskCreate 可靠性和遠程設置驗證。"
key_points:
  - "--safe-mode flag：一鍵禁用所有自訂化以隔離故障根源"
  - "/cd command：無損切換工作目錄，保留 prompt cache 避免 context 崩塌"
  - "修復 MCP 企業策略（allowedMcpServers/deniedMcpServers）多場景下未生效的問題（reconnect、首次啟動、IDE config）"
tags: [claude-code, safe-mode, mcp-policies, bug-fixes]
...
Claude Code v2.1.169 發布，新增三項關鍵功能：--safe-mode flag（禁用 CLAUDE.md、plugins、skills、hooks、MCP servers 以隔離故障），/cd command（在 session 中無損切換工作目錄並保留 prompt cache），disableBundledSkills 設定（隱藏內置 skills 和 slash commands）。修復 MCP 企業策略（allowedMcpServers/deniedMcpServers）在 reconnect、IDE 配置、首次啟動後未被正確執行的問題；修復 macOS 啟動時 ~30–50ms UI stall；修復 claude -p 在 Windows 上 slash-command 掃描緩慢的回歸。改進 TaskCreate 可靠性和遠程設置驗證。
...
**原文：** [claude-code-releases](https://github.com/anthropics/claude-code/releases/tag/v2.1.169)
